...
export async function POST(request) {
  try {
    ...
    logger.info('[Free Credit Migration] Starting migration...');
        ...
        if (statement.trim()) {
          try {
            ...
            if (SKIPPABLE_CODES.has(error.code)) {
              logger.info('[Free Credit Migration] Skipped existing object:', error.code);
              continue;
            }
            throw error;
          }
    ...
    logger.info('[Free Credit Migration] Migration completed successfully');
    ...
    logger.error('[Free Credit Migration] Error:', error);
    return NextResponse.json(
