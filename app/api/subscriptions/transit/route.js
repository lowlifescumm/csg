/**
 * Transit Subscription API
 * POST /api/subscriptions/transit - Create a transit monitoring subscription
 * GET /api/subscriptions/transit - Get user's transit subscriptions
 */
 ...
 * Create a new transit monitoring subscription
 * 
 * Body:
 * {
 *   "natalChartId": "uuid",
 *   "transitingBodies": ["Saturn", "Jupiter"],
...
export async function POST(req) {
  try {
    ...
    // Check if user is premium (required for transit monitoring)
    ...
    if (!isAdmin && !isPremium) {
      return NextResponse.json({
        error: 'Premium subscription required for transit monitoring',
    ...
    if (!natalChartId) {
      return NextResponse.json({
        error: 'Natal chart ID is required'
      }, { status: 400 });
    }

    // Verify natal chart ownership
    ...
    if (invalidPlanets.length > 0 || invalidNatalPoints.length > 0 || invalidAspects.length > 0) {
      return NextResponse.json({
    ...
    if (notifyWebhook && !webhookUrl) {
      return NextResponse.json({
        error: 'Webhook URL required when webhook notifications are enabled'
      }, { status: 400 });
    }
    ...
    return NextResponse.json({
      error: 'Failed to create transit subscription',
...
/**
 * GET /api/subscriptions/transit
 * Get all transit subscriptions for the authenticated user
 */
export async function GET(req) {
  try {
    ...
    return NextResponse.json({
      error: 'Failed to fetch transit subscriptions',
      details: error.message
    }, { status: 500 });
  }
}
...
/**
 * DELETE /api/subscriptions/transit/[id]
 * Delete a transit subscription
 */
export async function DELETE(req) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: 'Transit subscription deleted successfully'
    });
    ...
    return NextResponse.json({
      error: 'Failed to delete transit subscription',
      details: error.message
    }, { status: 500 });
  }
}
