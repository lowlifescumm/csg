/**
 * POST /api/marketplace/advisors/sessions/[sessionId]/webrtc/offer
 * Store WebRTC offer from one peer
 */
...
export async function POST(request, { params }) {
  try {
    ...
    if (!offer || typeof offer !== 'object') {
      return badRequestResponse('offer is required and must be an object');
    }

    if (!offer.type || offer.type !== 'offer') {
      return badRequestResponse('offer.type must be "offer"');
    }

    if (!offer.sdp || typeof offer.sdp !== 'string') {
      return badRequestResponse('offer.sdp is required and must be a string');
    }
