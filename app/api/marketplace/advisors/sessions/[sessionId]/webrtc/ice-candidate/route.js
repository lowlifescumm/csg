/**
 * POST /api/marketplace/advisors/sessions/[sessionId]/webrtc/ice-candidate
 * Store WebRTC ICE candidate from one peer
 */
...
export async function POST(request, { params }) {
  try {
    ...
    if (!candidate || typeof candidate !== 'object') {
      return badRequestResponse('candidate is required and must be an object');
    }

    // Validate candidate has required fields
    if (candidate.candidate === undefined && candidate.candidate === null) {
      return badRequestResponse('candidate.candidate is required');
    }
