...
export default function EditBlogPostPage() {
  ...
  const fetchPost = async () => {
    try {
      setLoading(true);
      const data = await apiClient.get(`/api/blog/${encodeURIComponent(String(slug))}?admin=true`);
      if (data.post) {
        setPost({
                ...
                <div>
                  ...
                  {htmlMode ? (
                    <div className="space-y-2">
                      ...
                      />
                      <p className="text-xs text-gray-500">HTML Mode: Paste raw HTML with images.</p>
                    </div>
                  ) : (
                    <textarea
                      value={post.content}
