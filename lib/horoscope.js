...
export async function generateDailyHoroscope(sign) {
  ...
  const luckyStone = {
    Aries:'Diamond',Taurus:'Emerald',Gemini:'Agate',Cancer:'Pearl',Leo:'Ruby',Virgo:'Sapphire',
    Libra:'Opal',Scorpio:'Topaz',Sagittarius:'Turquoise',Capricorn:'Garnet',Aquarius:'Amethyst',Pisces:'Aquamarine'
  }[signInfo.name] || 'Clear Quartz';
  const prompt = `Generate a daily horoscope for ${signInfo.name} (${signInfo.element} sign, ${signInfo.quality}).

Date: ${today}

Deterministic values (do not invent or recalculate these):
- Lucky Stone: ${luckyStone}
...
DO NOT use phrases like "As a ${signInfo.name}" or "Your sign". Write directly to the reader.`;
  ...
  const repairedText = text.trim() ? _completeHoroscopeText(text) : '';

  return {
    sign: signInfo.name,
    date: today,
    content: repairedText || text,
  };
}
...
export function _completeHoroscopeText(text) {
