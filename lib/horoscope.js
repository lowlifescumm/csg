...
export async function generateDailyHoroscope(sign) {
  ...
  const luckyNumbers = generateLuckyNumbers(signInfo.name.toLowerCase());
  const luckyColor = getLuckyColor(signInfo.name.toLowerCase());
...
Reference the lucky numbers/color naturally in the closing guidance if it fits, but do not state them as if you chose them.
