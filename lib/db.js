...
// Basic helpers
export async function saveReading({ userId, type, question, cards, interpretation, spreadType }) {
  const text = `
    INSERT INTO readings (user_id, type, question, result)
    VALUES ($1, $2, $3, $4)
  ...
  const { rows } = await pool.query(text, [userId, type, question, result]);
  return rows[0];
}
