...
export function generateBirthChartSVG(chartData, birthInfo = {}) {
  ...
  const moonPhasePanel = normalizedChart?.moonPhase ? `
    <g>
      ...
      <text x="1050" y="550" text-anchor="middle" font-size="28">${normalizedChart.moonPhase.emoji || '🌙'}</text>
      <text x="1050" y="572" text-anchor="middle" font-size="9" fill="#6b7280">${normalizedChart.moonPhase.name || ''}</text>
    </g>
  ` : '';
      ...
      ${normalizedChart.chartPatterns.slice(0,5).map((pattern, idx) => {
        ...
        return `
          <g>
            ...
            <text x="950" y="${62 + idx * 32}" font-size="10" fill="#374151" font-weight="600">${pattern.type}</text>
            <text x="950" y="${76 + idx * 32}" font-size="8" fill="#6b7280">
              ${(pattern.planets || []).slice(0,3).join(', ')}${pattern.planets && pattern.planets.length > 3 ? ` +${pattern.planets.length - 3}` : ''}
            </text>
          </g>
        `;
      }).join('')}
