...
export function generateBirthChartSVG(chartData, birthInfo = {}) {
  ...
  // Render planets
  const planets = chartData?.planets ? Object.entries(chartData.planets).map(([planet, data]) => {
  ...
  // Render houses
  const houses = chartData?.houses ? Object.entries(chartData.houses).map(([house, data]) => {
    if (!data || typeof data.longitude !== 'number' || isNaN(data.longitude)) {
      return '';
    }
  ...
  const partOfFortune = chartData?.partOfFortune && typeof chartData.partOfFortune.longitude === 'number' ? (() => {
