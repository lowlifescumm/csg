...
export async function generateTarotReading(cards, question, spread = "three-card", readingType = "general") {
  ...
  const spreadCfg = Array.isArray(spreads) ? JSON.parse(JSON.stringify(spreads.find(s => s.id === spreadId))) : null;
  
  if (spreadId === "custom_spread" && spreadCfg && cards && cards.length > 0) {
