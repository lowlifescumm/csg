...
export function getCachedReportData(reportType) {
  try {
    ...
    if (fs.existsSync(cachePath)) {
      ...
      console.log(`[Report Data Cache] ✓ Loaded cached data for ${reportType} (${Math.round(cachedData.length / 1024)}KB)`);
      return parsed;
    }
    
    return null;
  } catch (error) {
    console.warn(`[Report Data Cache] Error loading cache for ${reportType}:`, error.message);
    return null;
  }
}
...
export function setCachedReportData(reportType, data) {
  try {
    ...
    console.log(`[Report Data Cache] ✓ Saved cached data for ${reportType} (${sizeKB}KB)`);
  } catch (error) {
    console.warn(`[Report Data Cache] Error saving cache for ${reportType}:`, error.message);
  }
}
  ...
  try {
    if (reportType) {
      ...
      if (fs.existsSync(cachePath)) {
        ...
        console.log(`[Report Data Cache] ✓ Cleared cache for ${reportType}`);
      }
    } else {
      ...
      console.log(`[Report Data Cache] ✓ Cleared all cached report data`);
    }
  } catch (error) {
    console.warn(`[Report Data Cache] Error clearing cache:`, error.message);
  }
}
