...
// Try to use Playwright's Chromium as fallback (more reliable in this env)
// NOTE: Dynamic import to avoid Next.js webpack trying to bundle it server-side
let pw;
try {
  // pw = null; // Disabled - dynamic import approach below causes webpack issues
} catch (e) {
  pw = null;
}
...
export async function generatePremiumPdf(userData) {
  ...
  if (!userData.base64BackgroundImage && !userData.backgroundImageUrl) {
    ...
    logger.info('[Premium PDF Generator] Using default Cloudinary cover image:', defaultCloudinaryUrl);
    ...
    try {
      ...
      for (const testPath of possiblePaths) {
        if (fs.existsSync(testPath)) {
          bgPath = testPath;
          logger.info('[Premium PDF Generator] Found local fallback background image at:', bgPath);
          ...
          if (!process.env.DEFAULT_COVER_IMAGE_URL) {
            ...
            logger.info('[Premium PDF Generator] Using local fallback background image');
          }
          break;
        }
      }
    } catch (err) {
      logger.warn('[Premium PDF Generator] Could not load local fallback, using Cloudinary default');
    }
  } else {
    ...
    if (userData.backgroundImageUrl) {
      ...
      logger.info('[Premium PDF Generator] Using Cloudinary background image URL:', userData.backgroundImageUrl.substring(0, 50) + '...');
    } else if (userData.base64BackgroundImage) {
      logger.info('[Premium PDF Generator] Using provided background image (base64 or data URL)');
    }
  }
  ...
  // TASK 1: Use dynamic reportTitle instead of hardcoded 'Master Report'
  const wrapHtml = (htmlContent, sectionLabel = reportTitle) => {
...
<head>
  ...
  <title>${sectionLabel} - ${userData.name}</title>
...
<body>
  ...
  // TASK 3: Define Report "Recipes" - component lists for each report tier
  const REPORT_RECIPES = {
    essential: [
      { name: 'cover', label: 'Cover Page', hasBackground: true, type: 'html' },
      { name: 'birth_chart', label: 'Birth Chart', hasBackground: false, type: 'chart', landscape: true },
    ...
    master: [
      { name: 'cover', label: 'Cover Page', hasBackground: true, type: 'html' },
      { name: 'birth_chart', label: 'Birth Chart', hasBackground: false, type: 'chart', landscape: true },
      { name: 'partner_birth_chart', label: 'Partner Birth Chart', hasBackground: false, type: 'chart', landscape: true },
      { name: 'core_identity', label: 'Core Identity', hasBackground: false, type: 'html' },
      { name: 'planetary_analysis', label: 'Planetary Analysis', hasBackground: false, type: 'html' },
      { name: 'relationship_matrix', label: 'Relationship Matrix', hasBackground: false, type: 'html' },
      { name: 'compatibility', label: 'Compatibility Analysis', hasBackground: false, type: 'html' },
  ...
  // TASK 1: Determine report title based on reportType
  const getReportTitle = (reportType) => {
    ...
    return titleMap[normalizedType] || 'MASTER REPORT'; // Default to Master Report
  };

  // Get report type from userData (normalize to lowercase)
  const reportType = (userData.reportType || 'master').toLowerCase();
  const reportTitle = getReportTitle(reportType);

  // Select sections based on report recipe
  const sections = REPORT_RECIPES[reportType] || REPORT_RECIPES.master;
  
  logger.info(`[Premium PDF Generator] Using report type: ${reportType}, title: ${reportTitle}, sections: ${sections.length}`);
  ...
  if (chromium && (isLinux || isProduction)) {
    try {
      ...
      logger.warn('[Premium PDF Generator] Chromium executablePath failed, using system Chrome:', error);
    }
  }
  ...
  if (isWindows && !isProduction && !launchOptions.executablePath) {
    ...
    for (const chromePath of windowsChromePaths) {
      if (fs.existsSync(chromePath)) {
        ...
        logger.info('[Premium PDF Generator] Using Windows Chrome:', chromePath);
        break;
      }
    }
    ...
    if (!launchOptions.executablePath) {
      try {
        ...
        if (puppeteerExecutablePath && fs.existsSync(puppeteerExecutablePath)) {
          ...
          logger.info('[Premium PDF Generator] Using puppeteer.executablePath():', puppeteerExecutablePath);
        } else {
          logger.warn('[Premium PDF Generator] No Chrome found. Set PUPPETEER_EXECUTABLE_PATH env var or install Chrome.');
        }
      } catch (exePathError) {
        logger.warn('[Premium PDF Generator] puppeteer.executablePath() failed:', exePathError.message);
      }
    }
  }
  ...
  // Launch browser - try Puppeteer first, fall back to Playwright
  let browser;
  try {
    browser = await puppeteer.launch(launchOptions);
  } catch (launchError) {
    logger.warn('[Premium PDF Generator] Puppeteer launch failed, trying Playwright:', launchError.message);
    if (pw) {
      try {
        const pwChromium = pw.chromium;
        const execPath = pwChromium.executablePath();
        const pwBrowser = await pwChromium.launch({
          executablePath: execPath,
          headless: true,
          args: ['--no-sandbox', '--disable-setuid-sandbox'],
        });
        // Wrap Playwright browser in a Puppeteer-compatible interface
        browser = {
          newPage: async () => {
            const pwPage = await pwBrowser.newPage();
            return {
              setViewport: pwPage.setViewport.bind(pwPage),
              setContent: pwPage.setContent.bind(pwPage),
              pdf: pwPage.pdf.bind(pwPage),
              evaluate: pwPage.evaluate.bind(pwPage),
              close: pwBrowser.close.bind(pwBrowser),
        ...
        logger.info('[Premium PDF Generator] Playwright browser launched successfully');
      } catch (pwError) {
        logger.error('[Premium PDF Generator] Playwright launch also failed:', pwError.message);
        throw launchError;
      }
    } else {
      throw launchError;
  ...
  try {
    ...
    logger.info('[Premium PDF Generator] Starting Stitch Strategy - rendering sections individually');
    
    for (const section of sections) {
      try {
        logger.info(`[Premium PDF Generator] Rendering section: ${section.label}`);
        
        // HYBRID LAYOUT: Special handling for Birth Chart (landscape with embedded image)
        if (section.type === 'chart' && (section.name === 'birth_chart' || section.name === 'partner_birth_chart')) {
          ...
          const targetSectionType = section.name === 'partner_birth_chart' ? 'partner_birth_chart' : 'birth_chart';
          
          // Priority 1: Check sections for chartImage (from birth chart page - already perfectly sized)
          if (userData.sections) {
            const birthChartSection = userData.sections.find(s => s.type === targetSectionType);
            if (birthChartSection?.chartImage) {
              ...
              if (birthChartSection.chartImage.startsWith('data:image/png')) {
                ...
                logger.info('[Premium PDF Generator] Found PNG chart image from birth chart section (perfectly sized)');
                ...
                logger.info('[Premium PDF Generator] Found JPG chart image from birth chart section');
                ...
                logger.info('[Premium PDF Generator] Found SVG chart image from birth chart section (will render via HTML)');
              }
            }
          }
          ...
          if (!chartImageData && userData.chartImageBase64) {
            ...
            if (typeof imgData === 'string') {
              if (imgData.startsWith('data:image/png')) {
                ...
                chartImageFormat = 'png';
                logger.info('[Premium PDF Generator] Found PNG chart image from chartImageBase64');
          ...
          if (chartImageData && (chartImageFormat === 'png' || chartImageFormat === 'jpg')) {
            logger.info(`[Premium PDF Generator] Embedding ${chartImageFormat.toUpperCase()} chart image in landscape page`);
            ...
            logger.info(`[Premium PDF Generator] ? ${section.label} - added 1 landscape page with embedded image`);
            continue;
          } else if (chartImageFormat === 'svg' || (section.name === 'partner_birth_chart' ? userData.partnerBirthChartSvg : userData.birthChartSvg)) {
            // For SVG, render via HTML but in landscape orientation
            logger.info(`[Premium PDF Generator] Rendering SVG chart in landscape via HTML`);
            
            // TASK 1: Pass reportTitle to MasterReport component for dynamic cover page
            const sectionComponent = React.createElement(MasterReport, { 
              userData: { ...userData, reportTitle }, // Add reportTitle to userData
            ...
            if (!hasChartContent) {
              logger.info(`[Premium PDF Generator] Skipping ${section.label} - no SVG/chart content found (HTML length: ${sectionHtml?.length || 0})`);
              continue;
            }
            ...
            logger.info(`[Premium PDF Generator] ? ${section.label} - added ${pageIndices.length} landscape page(s)`);
            continue;
          } else {
            // No chart image found - skip this section entirely (don't create blank page)
            logger.info(`[Premium PDF Generator] No chart image found for ${section.label}, skipping section entirely`);
            continue;
          }
        }
        ...
        if (!hasContent) {
          logger.info(`[Premium PDF Generator] Skipping ${section.label} - no content (HTML length: ${sectionHtml?.length || 0})`);
          continue;
        }
        ...
        try {
          ...
          logger.info(`[Premium PDF Generator] networkidle0 wait failed, trying 'load' for ${section.label}...`);
          try {
            await page.setContent(fullSectionHtml, { 
              waitUntil: 'load',
              timeout: 60000
            ...
            logger.info(`[Premium PDF Generator] Load wait failed, trying domcontentloaded for ${section.label}...`);
            await page.setContent(fullSectionHtml, { 
        ...
        try {
          ...
          logger.warn(`[Premium PDF Generator] Font loading warning for ${section.label}:`, fontError.message);
        ...
        logger.info(`[Premium PDF Generator] ? ${section.label} - added ${pageIndices.length} page(s)`);
      } catch (sectionError) {
        logger.error(`[Premium PDF Generator] Error rendering section ${section.label}:`, sectionError.message || sectionError);
        ...
        if ((sectionError.name === 'TimeoutError' || sectionError.message?.includes('timeout')) && fullSectionHtml) {
          logger.info(`[Premium PDF Generator] Retrying ${section.label} with simpler wait condition...`);
          try {
    ...
    await new Promise(resolve => setTimeout(resolve, 1000));
      ...
      preferCSSPageSize: false,
            ...
            logger.info(`[Premium PDF Generator] ? ${section.label} - added ${pageIndices.length} page(s) (retry succeeded)`);
            continue;
          } catch (retryError) {
            logger.error(`[Premium PDF Generator] Retry failed for ${section.label}:`, retryError.message || retryError);
    ...
    logger.info('[Premium PDF Generator] Merging all sections into final PDF...');
    const finalPdfBytes = await mergedPdf.save();
    const finalBuffer = Buffer.from(finalPdfBytes);
    
    logger.info(`[Premium PDF Generator] ? PDF generation complete - ${finalBuffer.length} bytes`);
    return finalBuffer;
  } catch (error) {
    if (browser) {
      try {
