    ...
    for (const section of sections) {
      try {
        ...
        if (section.type === 'chart' && section.name === 'birth_chart') {
          // Check if we have chart image data
          const chartImageBase64 = userData.chartImageBase64 || 
                                   userData.birthChartImageBase64 ||
                                   (userData.birthChartSvg ? null : null); // Will handle SVG separately if needed
          
          // Try to find chart image from sections
          ...
          // Fallback: check chartImageBase64 directly
          if (!chartImageData && chartImageBase64) {
            if (typeof chartImageBase64 === 'string') {
              if (chartImageBase64.startsWith('data:image')) {
                chartImageData = chartImageBase64.split(',')[1];
                chartImageFormat = chartImageBase64.includes('png') ? 'png' : 
                                  chartImageBase64.includes('jpeg') || chartImageBase64.includes('jpg') ? 'jpg' : 'png';
              } else {
                chartImageData = chartImageBase64;
                chartImageFormat = 'png'; // Assume PNG if no data URL prefix
              }
            }
          }
