  ...
  for (const transitBody of TRANSITING_BODIES) {
    ...
    for (const natalPoint of NATAL_POINTS) {
      ...
      if (aspect) {
        ...
        if (strengthScore >= 30) {
          // Calculate exact date for conjunction
          let exactDate = null;
          if (aspect.type === 'conjunction') {
            exactDate = await calculateExactTime(
              transitBody,
              natalLong,
              0, // Conjunction angle
              referenceDate,
...
export async function calculateTransitsToHouseCusps(natalChart, referenceDate = new Date()) {
    ...
    for (let houseNum = 1; houseNum <= 12; houseNum++) {
      ...
      if (aspect) {
        ...
        if (aspect.type === 'conjunction' && aspect.orb <= 3) {
          ...
          const houseName = houseNames[houseNum] || `${houseNum}th House`;
  ...
  return cuspTransits.sort((a, b) => a.orb - b.orb); // Sort by tightest orb first
}
