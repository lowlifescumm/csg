...
const TRANSIT_DEDUP_WINDOW_DAYS = 3;
const DEFAULT_STRENGTH_THRESHOLD = 30;
...
/**
 * Check if a date is today or in the future (start-of-day comparison)
 */
 ...
 * @param {number} [options.strengthThreshold] - Minimum strength score (default: 30)
 * @returns {Array} Array of transit objects (filtered to future dates only)
 */
export async function calculateActiveTransits(natalChart, referenceDate = new Date(), options = {}) {
  const { strengthThreshold = DEFAULT_STRENGTH_THRESHOLD } = options;
  ...
  for (const transitBody of TRANSITING_BODIES) {
    ...
    for (const natalPoint of NATAL_POINTS) {
      ...
      if (aspect) {
        ...
        if (strengthScore >= strengthThreshold) {
          ...
          if (exactDate && isTodayOrFuture(exactDate)) {
            const transit = {
  ...
  const futureTransits = activeTransits.filter(t => {
    ...
    const valid = isTodayOrFuture(transitDate);
    if (!valid) {
      logger.warn(`[Transit Filter] Rejecting past date: ${transitDate}`);
    }
    return valid;
  });
...
export async function calculateTransitsToHouseCusps(natalChart, referenceDate = new Date()) {
  ...
  // Filter to ensure all dates are >= today
  const futureCuspTransits = cuspTransits.filter(t => {
    const transitDate = t.exactDate || t.date;
    const valid = isTodayOrFuture(transitDate);
    if (!valid) {
      logger.warn(`[Transit Filter] Rejecting past cusp transit date: ${transitDate}`);
    }
    return valid;
  });
...
export async function calculateAndStoreTransits(userId, natalChartId, natalChart, startDate = new Date(), windowDays = 90) {
  ...
  try {
    ...
    // Pre-cleanup: remove any duplicate transits before inserting new ones
    try {
      await deduplicateTransits(userId, natalChartId);
    } catch (dedupError) {
      logger.warn(`[Transit Dedup] Pre-insert cleanup failed (non-blocking): ${dedupError.message}`);
    }
    ...
    for (const sampleDate of samples) {
      ...
      for (const transit of transits) {
        ...
        // Check if transit already exists (wider dedup window)
        const { rows: existingRows } = await pool.query(
          `SELECT id FROM transits 
           ...
           AND natal_point = $4
           AND aspect = $5
           AND ABS(EXTRACT(EPOCH FROM (exact_time - $6::timestamp))) < $7`,
          [userId, natalChartId, transit.transitingBody, transit.natalPoint, transit.aspect, exactTime, TRANSIT_DEDUP_WINDOW_DAYS * 86400]
        );
    ...
    // Post-insert dedup: clean up any duplicates from the run
    try {
      const removed = await deduplicateTransits(userId, natalChartId);
      if (removed > 0) {
        logger.info(`[Transit Dedup] Post-insert cleanup removed ${removed} duplicates for user ${userId}`);
      }
    } catch (dedupError) {
      logger.warn(`[Transit Dedup] Post-insert cleanup failed (non-blocking): ${dedupError.message}`);
    }
...
/**
 * Generate a stable transit identity hash for deduplication.
 * Used to detect when the same transit (same user, chart, bodies, aspect) 
 * would be stored with slightly different exact_time values.
 */
export function generateTransitIdentityHash(userId, natalChartId, transitingBody, natalPoint, aspect) {
  const components = [String(userId), String(natalChartId), transitingBody, natalPoint, aspect].join('|');
  return crypto.createHash('sha256').update(components).digest('hex');
}

/**
 * Deduplicate stored transits by removing near-duplicate records.
 * Keeps the newer record and removes older duplicates.
 * @param {UUID} userId
 * @param {UUID} natalChartId
 * @returns {number} Number of duplicate records removed
 */
export async function deduplicateTransits(userId, natalChartId) {
  const { rows: duplicates } = await pool.query(`
    DELETE FROM transits
    WHERE id IN (
      SELECT t1.id FROM transits t1
      JOIN transits t2 ON (
        t1.user_id = t2.user_id
        AND t1.natal_chart_id = t2.natal_chart_id
        AND t1.transiting_body = t2.transiting_body
        AND t1.natal_point = t2.natal_point
        AND t1.aspect = t2.aspect
        AND t1.id <> t2.id
        AND ABS(EXTRACT(EPOCH FROM (t1.exact_time - t2.exact_time))) < $3
      )
      WHERE t1.user_id = $1 AND t1.natal_chart_id = $2
      AND t1.created_at > t2.created_at
    )
    RETURNING id
  `, [userId, natalChartId, TRANSIT_DEDUP_WINDOW_DAYS * 86400]);

  if (duplicates.length > 0) {
    logger.info(`[Transit Dedup] Removed ${duplicates.length} duplicate transits for user ${userId}`);
  }
