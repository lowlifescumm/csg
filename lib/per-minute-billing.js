...
export async function billSessionMinute(client, session) {
  try {
    ...
    // Edge cases handled:
    // - Balance exactly equals rate: Will allow this minute, terminate on next billing cycle
    // - Balance becomes negative: Will terminate immediately (balance < perMinuteRate catches this)
    // - Balance < amountToDebit: Will terminate (can't afford the minutes to bill)
    // - Balance < perMinuteRate: Will terminate (can't afford even 1 minute)
    const hasInsufficientFunds = userBalance < perMinuteRate || userBalance < amountToDebit;
    
    if (hasInsufficientFunds) {
      // Release the transaction before calling finalizeSessionBilling (which uses its own transaction)
      await client.query('ROLLBACK');
      
      // Auto-terminate session due to insufficient funds
      console.log(`[Per-Minute Billing] Insufficient funds detected for session ${lockedSession.id}. Auto-terminating...`);
      console.log(`[Per-Minute Billing] Balance: $${userBalance.toFixed(2)}, Required: $${amountToDebit.toFixed(2)}, Rate: $${perMinuteRate.toFixed(2)}`);
      
      try {
        const terminationResult = await finalizeSessionBilling(
          lockedSession.id, 
          lockedSession.user_id,
          'insufficient_funds'
        );
        ...
        if (terminationResult.success) {
          console.log(`[Per-Minute Billing] Session ${lockedSession.id} terminated due to insufficient funds`);
          return {
            success: false,
            error: `Insufficient funds - session terminated. Balance: $${userBalance.toFixed(2)}, Required: $${amountToDebit.toFixed(2)}`,
            terminated: true
          };
        } else {
          console.error(`[Per-Minute Billing] Failed to terminate session ${lockedSession.id}:`, terminationResult.error);
          return {
            success: false,
            error: `Insufficient funds and termination failed: ${terminationResult.error}`
          };
        }
      } catch (terminationError) {
        console.error(`[Per-Minute Billing] Error terminating session ${lockedSession.id}:`, terminationError);
        return {
          success: false,
          error: `Insufficient funds. Balance: $${userBalance.toFixed(2)}, Required: $${amountToDebit.toFixed(2)}. Termination error: ${terminationError.message}`
        };
      }
    }
