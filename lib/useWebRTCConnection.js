  ...
  const createPeerConnection = useCallback(() => {
    ...
    pc.onconnectionstatechange = async () => {
      ...
      if (state === 'connected' || state === 'completed') {
        ...
        // Auto-disconnect session when WebRTC connection closes
        // Only if session is still ACTIVE (idempotent check on server)
        try {
          await fetch(`/api/marketplace/advisors/sessions/${sessionId}/disconnect`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' }
          });
          console.log('[WebRTC] Session disconnected due to connection loss');
        } catch (error) {
          console.error('[WebRTC] Failed to disconnect session:', error);
        }
