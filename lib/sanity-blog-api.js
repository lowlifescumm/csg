...
function buildImageUrl(image) {
  ...
  // Append ?fm=<ext>: without it Sanity CDN returns content-type: application/json,
  // which browsers block cross-origin via ORB (ERR_BLOCKED_BY_ORB) -> images never load.
  const fmt = (ref.match(/-(webp|jpg|jpeg|png|gif)$/i) || [, 'webp'])[1].toLowerCase();
  return `${SANITY_CDN}/${assetPath}?fm=${fmt}`;
}
