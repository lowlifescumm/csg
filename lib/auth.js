...
export async function getUserByEmail(email) {
  ...
  const normalizedEmail = email.toLowerCase().trim();
  const { rows } = await pool.query(
    'SELECT id, email, password_hash as password, first_name, last_name, role, created_at FROM users WHERE LOWER(email) = $1',
    [normalizedEmail]
  );
  return rows[0];
}
  ...
  try {
    ...
    if (authOptions) {
      try {
        ...
        if (session?.user?.id) {
          // #region agent log
          fetch('http://127.0.0.1:7242/ingest/36ab7c16-0814-43e1-a364-65e843241344',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'auth.js:131',message:'NextAuth session userId',data:{userId:session.user.id,userIdType:typeof session.user.id,isNaN:isNaN(session.user.id)},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
          // #endregion
          return {
    ...
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/36ab7c16-0814-43e1-a364-65e843241344',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'auth.js:157',message:'JWT decoded userId',data:{userId:decoded.userId,userIdType:typeof decoded.userId,isNaN:isNaN(decoded.userId)},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
    // #endregion
    ...
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/36ab7c16-0814-43e1-a364-65e843241344',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'auth.js:168',message:'getAuthenticatedUser return JWT',data:{userId:decoded.userId,userIdType:typeof decoded.userId,userDbId:user.id,userDbIdType:typeof user.id},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
    // #endregion
