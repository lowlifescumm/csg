...
export async function getAuthenticatedUser(cookies, authOptions = null) {
  try {
    ...
    if (authOptions && cookies) {
      // Build cookies string from cookies object
      let cookieString = '';
      if (cookies && typeof cookies.getAll === 'function') {
        // cookies() object from next/headers
        const allCookies = cookies.getAll();
        cookieString = allCookies.map(c => `${c.name}=${c.value}`).join('; ');
      } else if (cookies && cookies.toString) {
        // Headers object with cookies
        cookieString = cookies.toString();
      }
      
      if (cookieString) {
        const session = await getServerSession(authOptions, { req: { headers: { cookie: cookieString } } });
        if (session?.user?.id) {
          return {
