...
export function getRetrogradeStatus(date = new Date()) {
  ...
  for (const planetName of RETROGRADE_PLANETS) {
    ...
    const todayVec = Astronomy.GeoVector(planetName, today, false);
    const tomorrowVec = Astronomy.GeoVector(planetName, tomorrow, false);
    
    const todayEcl = Astronomy.Ecliptic(todayVec);
    const tomorrowEcl = Astronomy.Ecliptic(tomorrowVec);
    
    let diff = tomorrowEcl.elon - todayEcl.elon;
    if (diff > 180) diff -= 360;
    if (diff < -180) diff += 360;
    
    retrogradeStatus[planetName.toLowerCase()] = diff < 0;
  }
...
export function getCurrentPlanetaryPositions(date = new Date()) {
  ...
  const retrogradeStatus = getRetrogradeStatus(date);
  ...
  for (const [key, data] of Object.entries(planets)) {
    ...
    positions[key] = {
      ...
      name: data.name,
      isRetrograde: retrogradeStatus[key] || false
    };
  }
...
export function calculateActiveTransits(userBirthChart, date = new Date()) {
...
const SLOW_TRANSIT_PLANETS = ['jupiter', 'saturn', 'uranus', 'neptune', 'pluto'];

export function isSlowTransitPlanet(planet) {
  return SLOW_TRANSIT_PLANETS.includes(planet);
}

export function getTransitKey(transit) {
  return `${transit.transitPlanet}-${transit.natalPlanet}-${transit.aspect}`;
}

export function mergeContinuousTransits(transits) {
  const mergedMap = new Map();

  for (const transit of transits) {
    if (isSlowTransitPlanet(transit.transitPlanet)) {
      const key = getTransitKey(transit);

      if (mergedMap.has(key)) {
        const existing = mergedMap.get(key);
        if (transit.orb < existing.orb) {
          mergedMap.set(key, {
            ...transit,
            continuous: true,
            mergedFrom: existing
        ...
        mergedMap.set(key, { ...transit, continuous: false });
      }
    } else {
      mergedMap.set(`${getTransitKey(transit)}-${Math.random()}`, { ...transit, continuous: false });
    }
  }

  return Array.from(mergedMap.values()).sort((a, b) => b.intensity - a.intensity);
}

export function filterRoutineTransits(transits, options = {}) {
  const {
    minIntensity = 4,
    keepLunarConjunctions = true,
    keepLunarToSunMoon = true,
  } = options;

  return transits.filter(transit => {
    if (transit.natalPlanet === 'moon') {
      if (keepLunarToSunMoon && transit.transitPlanet === 'sun') return true;
      if (keepLunarConjunctions && transit.aspect === 'conjunction') return true;
      if (transit.intensity >= minIntensity) return true;
      return false;
    }
    return true;
  });
