 ...
 */
function buildPlanetSignHouseCombinations(chart, planetHouses) {
  const combinations = [];
  const planetNames = {
  ...
  for (const [planetKey, planetData] of Object.entries(chart)) {
    ...
    const house = planetHouses[planetKey] || planetHouses[planetName.toLowerCase()];
...
function buildHouseCuspsDetailed(houses) {
  ...
  for (let i = 1; i <= 12; i++) {
    if (houses[i] && houses[i].longitude !== undefined) {
      ...
      const houseName = houseNames[i] || `${i}th House`;
...
function getChartRulerLocation(chartRuler, chart, planetHouses) {
  ...
  return {
    ...
    house: rulerHouse,
    houseName: houseNames[rulerHouse] || `${rulerHouse}th House`,
    degree: Math.floor(rulerPlanet.degree),
    description: `${chartRuler} in ${rulerPlanet.sign} in the ${houseNames[rulerHouse] || `${rulerHouse}th House`}`,
    fullDescription: `${chartRuler} (${chartRuler} as chart ruler) in ${rulerPlanet.sign} at ${Math.floor(rulerPlanet.degree)}° in the ${houseNames[rulerHouse] || `${rulerHouse}th House`}`
  };
}
