...
export function calculateBirthChart(birthDate, birthTime, latitude, longitude) {
  const parsedLatitude = typeof latitude === 'number' ? latitude : parseFloat(latitude || '0');
  const parsedLongitude = typeof longitude === 'number' ? longitude : parseFloat(longitude || '0');
  ...
  const houses = calculateHouses(datetime, parsedLatitude, parsedLongitude);
...
export function assignPlanetsToHouses(chart, houses) {
  ...
  if (houseCusps.length === 0) {
    logger.warn('[assignPlanetsToHouses] No house cusps found');
    return planetHouses;
  }
  ...
  for (const [planet, data] of Object.entries(chart)) {
    ...
    if (house !== null) {
      ...
      logger.warn(`[assignPlanetsToHouses] Could not assign house for ${planet} at longitude ${planetLon}`);
    }
  }
