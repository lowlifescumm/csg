...
export async function markJobSucceeded(jobId, readingResultId) {
  ...
  try {
    ...
    await client.query('ROLLBACK');
    console.error('[JobQueue] Error marking job succeeded:', error);
    throw error;
  } finally {
    client.release();
  }
  ...
  try {
    ...
    if (job.ledger_entry_id && !job.refund_ledger_id && job.charge_amount) {
      ...
      if (!refundResult.success) {
        console.error('[JobQueue] Failed to refund credits for job', jobId, refundResult.error);
      }
    }
    ...
    await client.query('ROLLBACK');
    console.error('[JobQueue] Error marking job failed:', error);
    throw error;
  } finally {
    client.release();
  }
...
export async function processJob(job, jobProcessor) {
  ...
  try {
    console.log(`[JobQueue] Processing job ${jobId} (${readingType})`);
    ...
    const { rows: resultRows } = await pool.query(
      ...
      RETURNING id`,
      [userId, jobId, readingType, JSON.stringify(result.content || {}), 'completed']
    );
...
export async function startWorker(queue = 'default', jobProcessor, options = {}) {
  ...
  console.log(`[JobQueue] Starting worker for queue: ${queue}`);
    ...
    try {
      ...
      for (const job of jobsToProcess) {
        ...
        processJob(job, jobProcessor)
          .then(() => {
            console.log(`[JobQueue] Job ${job.id} processed successfully`);
          })
          .catch((error) => {
            console.error(`[JobQueue] Job ${job.id} processing error:`, error);
          })
          .finally(() => {
  ...
  return () => {
    isRunning = false;
    console.log(`[JobQueue] Worker stopped for queue: ${queue}`);
  };
}
