...
export function getCardBySlug(slug) {
  return ALL_CARDS.find((card) => cardSlug(card.name) === slug) || null;
}

export function getCardSuit(slug) {
  const card = getCardBySlug(slug);
  if (!card) return null;
  if (MAJOR_ARCANA.some(c => c.id === card.id)) return 'Major Arcana';
  for (const [suit, cards] of Object.entries(MINOR_ARCANA)) {
    if (cards.some(c => c.id === card.id)) return suit;
  }
  return null;
}
