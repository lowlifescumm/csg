...
import spreads from "./tarot-spreads.json" with { type: "json" };
...
export function drawCards(count = 3) {
  ...
  if (deck.length < safeCount) {
    logger.error("Invalid deck or not enough cards to draw", { requested: safeCount, available: deck.length });
    return [];
  }
...
/**
 * Generate a URL-friendly slug from a tarot card name.
 * "The Fool" → "the-fool", "Ace of Wands" → "ace-of-wands"
 */
export function slugFromCardName(name) {
  return name
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-|-$/g, "");
...
const ALL_CARDS_WITH_SLUGS = ALL_CARDS.map((card) => ({
  ...card,
  slug: slugFromCardName(card.name),
}));

/**
 * Find a tarot card by its URL slug.
 */
export function findCardBySlug(slug) {
  return ALL_CARDS_WITH_SLUGS.find((card) => card.slug === slug) || null;
}
...
export function getAllCardSlugs() {
  return ALL_CARDS_WITH_SLUGS.map((card) => card.slug);
}
