...
export async function generatePdfFromHtml(html, options = {}) {
  ...
  // Launch browser with chromium configuration (exact same as existing generatePDF)
  // Handle both function and property access for executablePath (version compatibility)
  let executablePath;
  if (typeof chromium.executablePath === 'function') {
    ...
    // Fallback: try to get it from the default export
    executablePath = chromium.default?.executablePath || chromium.executablePath;
  }
  ...
  const browser = await puppeteer.launch({
    ...
    headless: chromium.headless !== false, // Default to true if not specified
  });
...
export async function uploadPdfToCloudinary(pdfBuffer, reportType, options = {}) {
  const { v2: cloudinary } = await import('./cloudinary.js');
