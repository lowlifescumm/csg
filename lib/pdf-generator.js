  ...
  <style>
    ...
    * {
    ...
    .section-media::after {
      ...
      inset: 0;
      border-radius: 30px;
      background: linear-gradient(145deg, rgba(255,255,255,0.15), rgba(15,23,42,0.35));
  ...
  try {
    ...
    if (reportType === 'ESSENTIAL') {
      ...
      try {
        ...
        // Ensure chart data is enriched and normalized before generating SVG
        const enrichedChartData = await enrichBirthChartData(data.birth_chart_data);
        
        const chartSVG = generateBirthChartSVG(
          enrichedChartData || data.birth_chart_data,
          {
