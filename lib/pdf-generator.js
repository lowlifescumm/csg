...
export async function generatePDF(reportType, reportData, content, options = {}) {
  try {
    ...
    try {
          ...
          if (birthChartSection.chartImage.startsWith('data:image/svg+xml')) {
            ...
            if (base64Match) {
              try {
                ...
                console.warn('[PDF Generator] Could not decode birth chart SVG');
              }
            }
          }
        }
      ...
      console.log('[PDF Generator] Premium PDF generated and uploaded successfully:', pdfUrl);
    } catch (premiumError) {
      console.warn('[PDF Generator] Premium PDF generation failed, falling back to standard generator:', premiumError.message);
      // Fallback to standard Puppeteer generator
      try {
        const pdfBuffer = await generatePdfFromHtml(html);
        pdfUrl = await uploadPdfToCloudinary(pdfBuffer, reportType);
        console.log('[PDF Generator] Standard PDF generated and uploaded successfully:', pdfUrl);
      } catch (puppeteerError) {
        console.error('[PDF Generator] Standard Puppeteer PDF generation also failed:', puppeteerError);
  ...
  try {
    ...
    if (reportType === 'ESSENTIAL') {
      ...
      try {
        ...
        console.error('[PDF Generator] Error generating partner birth chart image:', partnerChartError);
      }
    ...
    if (!data.skipPdf) {
      ...
      let compatibilityChartSvg = null;
        ...
        if (partnerBirthChartSection?.chartImage && partnerBirthChartSection.chartImage.startsWith('data:image/svg+xml')) {
          ...
          if (base64Match) {
            try {
              ...
              console.warn('[PDF Generator] Could not decode partner birth chart SVG');
            }
          }
        }
        ...
        console.error('[PDF Generator] Premium PDF generation failed, falling back to standard generator:', premiumError);
