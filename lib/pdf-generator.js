...
export async function generatePDF(reportType, reportData, content, options = {}) {
  try {
    ...
    try {
      // Use Puppeteer with @sparticuz/chromium for Render.com compatibility
      const chromium = await import('@sparticuz/chromium');
      const puppeteer = await import('puppeteer-core');
      
      console.log('[PDF Generator] Using Puppeteer with @sparticuz/chromium');
      
      // Configure chromium for Lambda/Render environments
...
<head>
  ...
  <style>
    ...
    @page {
      ...
      margin: 0.8in; /* Master margin for the document */
    ...
    * {
    ...
    /* 4. Backgrounds - Move to body so it repeats on every page automatically */
    body {
    ...
    /* 2. The Container is now a wrapper, not a fixed box */
    .page-section {
      width: 100%;
      max-width: 7in;
      margin: 0 auto;
      position: relative;
      /* Do NOT set height: 11in here. Let it grow. */
      /* Do NOT set overflow: hidden. */
      page-break-after: always; /* Force start of NEXT section on new page */
      padding: 0.4in 0;
    }
    ...
    /* Legacy .page class for cover page only */
    .page {
      width: 8.5in;
      height: 11in;
      position: relative;
    ...
    /* Simple page section styling - content flows naturally */
    .page-section {
      padding: 0.3in 0;
    }
    
    /* 3. Handle Breaks intelligently */
    h1, h2, .section-title {
    ...
    .section {
      ...
      /* Allow sections to span multiple pages naturally */
    ...
    .section-body {
      ...
      position: relative;
      /* Allow section-body to span multiple pages for long content */
      page-break-inside: auto;
    ...
    /* 4. Paragraph flow with intelligent breaks */
    p {
...
<body>
  ...
  <!-- Content Pages with Flow Layout -->
