...
const { validateReportConsistency } = require('./report-consistency-validator');
const { deduplicateSections, auditSections } = require('./paragraph-audit');
/**
...
import { scrubTemplateArtifacts } from './template-scrub.js';
...
async function enrichBirthChartData(rawData) {
  ...
  if (!premiumData.planetSignHouseCombinations || !premiumData.houseCuspsDetailed || isIncomplete) {
    try {
      ...
      logger.error('[Data Enrichment] Error recalculating chart:', error);
    }
  }
  ...
  if (!hasAllCritical && normalizedPlanets && normalizedHouses) {
    logger.warn('[Data Enrichment] Missing critical planets in combinations, attempting manual build');
    try {
      const { buildPlanetSignHouseCombinations, assignPlanetsToHouses } = await import('./astrology.js');
      const planetHouses = assignPlanetsToHouses(normalizedPlanets, normalizedHouses);
      ...
      logger.error('[Data Enrichment] Error building combinations manually:', error);
    }
  }
...
async function enrichCompatibilityData(rawData) {
  ...
  if ((!premiumData.synastryAspects || !premiumData.houseOverlays || !premiumData.compositeChart) 
    ...
    try {
      ...
      logger.error('[Data Enrichment] Error calculating compatibility:', error);
    }
  }
...
export async function generateReportContent(reportType, data, progressCallback) {
  try {
    ...
    await progressCallback?.(75, 'Scrubbing template artifacts...');

    const scrubbedContent = scrubTemplateArtifacts(content);

    await progressCallback?.(80, 'Formatting report...');
    
    return {
      content: scrubbedContent,
      sections: parseReportSections(scrubbedContent, reportType),
    };
  } catch (error) {
    logger.error('[PDF Generator] Error generating report content:', error);
    throw error;
  }
}
...
export async function generatePdfFromHtml(html, options = {}) {
  ...
  logger.info('[PDF Generator] Using Puppeteer with @sparticuz/chromium');
  ...
  try {
    ...
    if (typeof chromium.executablePath === 'function') {
      try {
        ...
        logger.warn('[PDF Generator] Chromium executablePath() failed:', execPathError.message);
        logger.warn('[PDF Generator] Falling back to system Chrome/Chromium');
      ...
      try {
        ...
        logger.warn('[PDF Generator] Chromium default executablePath() failed:', execPathError.message);
        logger.warn('[PDF Generator] Falling back to system Chrome/Chromium');
      }
    }
    ...
    if (executablePath) {
      ...
      logger.info('[PDF Generator] Using Chromium executablePath:', executablePath);
    } else {
      // Fallback: let Puppeteer find Chrome/Chromium automatically (for local dev or when brotli files missing)
      logger.warn('[PDF Generator] No chromium executablePath available, using system Chrome/Chromium');
    }
  } catch (error) {
    // Catch any other errors during chromium setup
    logger.warn('[PDF Generator] Error setting up Chromium, using system Chrome:', error.message);
  ...
  try {
    ...
    logger.info('[PDF Generator] HTML length:', html.length, 'characters');
    if (html.length < 100) {
      logger.warn('[PDF Generator] WARNING: HTML is very short, might be empty or malformed');
      logger.info('[PDF Generator] HTML preview (first 500 chars):', html.substring(0, 500));
    }
    ...
    if (pdfHeader !== '%PDF') {
      logger.error('[PDF Generator] WARNING: PDF buffer does not start with PDF magic bytes:', pdfHeader);
      logger.info('[PDF Generator] First 100 bytes:', pdfBuffer.toString('hex', 0, 100));
    } else {
      logger.info('[PDF Generator] PDF buffer is valid, size:', pdfBuffer.length, 'bytes');
    }
...
export async function uploadPdfToCloudinary(pdfBuffer, reportType, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    const uploadStream = cloudinary.uploader.upload_stream(
      ...
      (error, result) => {
        if (error) {
          logger.error('[PDF Generator] Cloudinary upload error:', error);
          reject(error);
        } else {
...
/**
 * Generate PDF from report content using Premium PDF Generator by default
 * Falls back to standard Puppeteer generator if premium generator fails
 */
export async function generatePDF(reportType, reportData, content, options = {}) {
  try {
    ...
    try {
      // Try premium PDF generator first
      const { generatePremiumPdf } = await import('./premium-pdf-generator.js');
      const { uploadPdfToCloudinary } = await import('./pdf-utils.js');
      
      // Convert report data to premium generator format
      const sections = content?.sections || [];
      const reportContent = content?.content || '';
      
      // Extract chart SVGs if available
      let birthChartSvg = null;
      let partnerBirthChartSvg = null;
      let compatibilityChartSvg = null;
      let compositeChartSvg = null;
      
      if (sections.length > 0) {
        const birthChartSection = sections.find(s => s.type === 'birth_chart');
          ...
          if (birthChartSection.chartImage.startsWith('data:image/svg+xml')) {
            ...
            if (base64Match) {
              try {
                ...
                logger.warn('[PDF Generator] Could not decode birth chart SVG');
              }
            }
          }
        }

        const partnerBirthChartSection = sections.find(s => s.type === 'partner_birth_chart');
        if (partnerBirthChartSection?.chartImage) {
          if (partnerBirthChartSection.chartImage.startsWith('data:image/svg+xml')) {
            const base64Match = partnerBirthChartSection.chartImage.match(/base64,(.+)/);
            if (base64Match) {
              try {
                partnerBirthChartSvg = Buffer.from(base64Match[1], 'base64').toString('utf-8');
                if (!partnerBirthChartSvg.includes('xmlns=')) {
                  partnerBirthChartSvg = partnerBirthChartSvg.replace('<svg', '<svg xmlns="http://www.w3.org/2000/svg"');
                }
              } catch (e) {
                logger.warn('[PDF Generator] Could not decode partner birth chart SVG');
              }
            }
          }
        }
        
        const matrixSection = sections.find(s => s.type === 'matrix');
        ...
        const compatibilitySection = sections.find(s => s.type === 'compatibility');
        if (compatibilitySection?.compositeChartSvg) {
          compositeChartSvg = compatibilitySection.compositeChartSvg;
          if (!compositeChartSvg.includes('xmlns=')) {
            compositeChartSvg = compositeChartSvg.replace('<svg', '<svg xmlns="http://www.w3.org/2000/svg"');
          }
        }
      }
      
      // Prepare userData for premium generator
      const natalChart = reportData.natalChart || reportData.birth_chart_data;
      const userData = {
        name: reportData.name || 'Beloved Seeker',
        birthDate: natalChart?.birth_date || reportData.birth_date || '',
        birthTime: natalChart?.birth_time || reportData.birth_time || '',
        location: natalChart?.location || reportData.location || '',
        sunSign: natalChart?.sun?.sign || reportData.sun_sign || '',
        moonSign: natalChart?.moon?.sign || reportData.moon_sign || '',
        risingSign: natalChart?.rising?.sign || natalChart?.ascendant?.sign || reportData.rising_sign || '',
        birthChartSvg: birthChartSvg,
        partnerBirthChartSvg: partnerBirthChartSvg,
        compatibilityChartSvg: compatibilityChartSvg,
        compositeChartSvg: compositeChartSvg,
        sections: sections.length > 0 ? sections.map(s => ({
          type: s.type,
          title: s.title,
          content: typeof s.content === 'string' ? s.content : (s.content?.content || ''),
          chartImage: s.chartImage || null,
        })) : [{
          type: reportType,
          title: reportType.charAt(0).toUpperCase() + reportType.slice(1).replace(/_/g, ' '),
          content: reportContent,
        }],
        compatibilityScores: reportData.matrix_scores || reportData.chartData?.matrix_scores || reportData.compatibility_data?.matrix_scores,
      };
      
      // Generate PDF using premium generator
      const pdfBuffer = await generatePremiumPdf(userData);
      
      // Upload to Cloudinary
      pdfUrl = await uploadPdfToCloudinary(pdfBuffer, reportType, {
        folder: 'reports',
        public_id: `report-${reportType}-premium-${Date.now()}-${Math.random().toString(36).substring(7)}`,
      });
      
      logger.info('[PDF Generator] Premium PDF generated and uploaded successfully:', pdfUrl);
    } catch (premiumError) {
      logger.warn('[PDF Generator] Premium PDF generation failed, falling back to standard generator:', premiumError.message);
      // Fallback to standard Puppeteer generator
      try {
        const pdfBuffer = await generatePdfFromHtml(html);
        pdfUrl = await uploadPdfToCloudinary(pdfBuffer, reportType);
        logger.info('[PDF Generator] Standard PDF generated and uploaded successfully:', pdfUrl);
      } catch (puppeteerError) {
        logger.error('[PDF Generator] Standard Puppeteer PDF generation also failed:', puppeteerError);
    ...
    logger.error('[PDF Generator] Error generating PDF:', error);
...
/**
 * Normalize composite chart data from either the new chartHydrator format or the legacy
 * compatibility.js format into the CompositeChartInput format expected by generateCompositeChartSVG.
 * 
 * New format: { sun: { sign, house, longitude, degree }, moon: { ... }, ..., rising: { sign, longitude, degree } }
 * Legacy format: { planets: { sun: { sign, degree, longitude }, ... }, planetHouses: { sun: 1, ... }, ... }
 * 
 * @param {object} data - Composite chart data in either format
 * @returns {object|null} Normalized CompositeChartInput or null if data is unusable
 */
function normalizeCompositeData(data) {
  if (!data) return null;

  // Handle new format (from chartHydrator.ts): { sun: { sign, house, longitude, degree }, moon: {...}, ... }
  if (data.sun && data.rising && typeof data.sun.longitude === 'number') {
    const PLANET_KEYS = ['sun', 'moon', 'mercury', 'venus', 'mars', 'jupiter', 'saturn'];
    const PLANET_LABELS = { sun: 'Sun', moon: 'Moon', mercury: 'Mercury', venus: 'Venus', mars: 'Mars', jupiter: 'Jupiter', saturn: 'Saturn' };
    // Also try to include uranus, neptune, pluto if available
    const EXTRA_KEYS = ['uranus', 'neptune', 'pluto'];

    const planets = [];
    for (const key of [...PLANET_KEYS, ...EXTRA_KEYS]) {
      const p = data[key];
      if (p && typeof p.longitude === 'number' && p.sign && p.sign !== 'Unknown') {
        planets.push({
          name: PLANET_LABELS[key] || key.charAt(0).toUpperCase() + key.slice(1),
          sign: p.sign,
          degree: typeof p.degree === 'number' ? p.degree : p.longitude % 30,
    ...
    return {
      planets,
      rising: {
        sign: data.rising.sign || 'Aries',
        longitude: typeof data.rising.longitude === 'number' ? data.rising.longitude : 0,
        degree: typeof data.rising.degree === 'number' ? data.rising.degree : 0,
      },
    };
  }

  // Handle legacy format (from compatibility.js): { planets: { sun: { sign, degree, longitude }, ... }, planetHouses: {...} }
  if (data.planets && typeof data.planets === 'object') {
    ...
    for (const [key, p] of Object.entries(data.planets)) {
      ...
      if (p && typeof p.longitude === 'number' && p.sign) {
        planets.push({
          name: PLANET_LABELS[lowerKey] || key.charAt(0).toUpperCase() + key.slice(1),
          sign: p.sign,
          degree: typeof p.degree === 'number' ? p.degree : p.longitude % 30,
          longitude: p.longitude,
          house: data.planetHouses?.[lowerKey] || data.planetHouses?.[key] || 1,
        });
      }
    }

    // Calculate rising from house 1 cusp if available
    let rising = { sign: 'Aries', longitude: 0, degree: 0 };
    if (data.houses?.[1] && typeof data.houses[1].longitude === 'number') {
      rising = {
        sign: data.houses[1].sign || 'Aries',
        longitude: data.houses[1].longitude,
        degree: typeof data.houses[1].degree === 'number' ? data.houses[1].degree : data.houses[1].longitude % 30,
      };
    }

    if (planets.length === 0) return null;

    return { planets, rising };
  }
  ...
  try {
    ...
    if (natalChart && (reportType === 'ADVANCED' || reportType === 'MASTER')) {
      ...
      try {
        ...
        // Log error and abort
        logger.error('[PDF Generator] Validation failed, aborting generation:', validationError.message);
    ...
    if (reportType === 'ESSENTIAL') {
      ...
      // Advanced: 7 Deep Analysis Sections + Compatibility + Extended Forecast
      // All sections use the same natalChart object
      await progressCallback?.(10, 'Preparing Advanced report data...');
      ...
      try {
        ...
        logger.error('[PDF Generator] Error generating birth chart image:', chartError);
      }

      // STEP 1: Generate all 7 deep analysis sections in parallel with individual error handling
      await progressCallback?.(15, 'Generating deep analysis sections...');
      const deepAnalysisConfigs = [
        { type: 'advanced_houses', title: 'Planetary Houses', reportType: 'advanced_houses' },
        { type: 'advanced_aspects', title: 'Aspect Interpretations', reportType: 'advanced_aspects' },
        { type: 'advanced_career', title: 'Career Path', reportType: 'advanced_career' },
        { type: 'advanced_relationships', title: 'Relationship Insights', reportType: 'advanced_relationships' },
        { type: 'advanced_life_purpose', title: 'Life Purpose', reportType: 'advanced_life_purpose' },
        { type: 'advanced_financial', title: 'Financial Outlook', reportType: 'advanced_financial' },
        { type: 'advanced_health', title: 'Health & Wellness', reportType: 'advanced_health' },
      ];

      const deepAnalysisResults = await Promise.all(
        deepAnalysisConfigs.map(async (config) => {
          try {
            const sectionData = await generateReportContent(config.reportType, {
              name: data.name,
              natalChart: advancedNatalChart,
            });
            return {
              type: config.type,
              title: config.title,
              content: sectionData,
              summary: extractKeyPoints(sectionData.content, config.type),
            };
          } catch (sectionError) {
            logger.error(`[PDF Generator] Error generating ${config.type} section:`, sectionError.message);
            // Return a fallback section so the report still completes
            return {
              type: config.type,
              title: config.title,
              content: { content: `We encountered an issue generating your ${config.title.toLowerCase()} section. Please contact support if this persists.` },
              summary: `${config.title} analysis`,
            };
          }
        })
      );

      // Attach chart image to the first deep analysis section (houses)
      if (chartImageUrl && deepAnalysisResults.length > 0) {
        deepAnalysisResults[0].chartImage = chartImageUrl;
      }

      sections.push(...deepAnalysisResults);
      await progressCallback?.(55, 'Deep analysis sections complete.');

      // STEP 2: Compatibility
      await progressCallback?.(60, 'Generating Compatibility report...');
      const compatibilityData = {
        ...data.compatibility_data,
        user: advancedNatalChart,
      ...
      // STEP 3: Extended Transit Forecast
      await progressCallback?.(80, 'Generating Extended forecast...');
      const forecastData = {
        ...data.transit_data,
        natalChart: advancedNatalChart,
        name: data.name,
      ...
      // All independent sections are generated in parallel for ~3x speedup
      const resolvedPartnerChart =
        data.compatibility_data?.partner ||
        data.chartData?.partner ||
        data.matrix_data?.partner ||
        data.matrix_data?.chart2 ||
        data.matrix_data?.pair?.partner ||
        null;

      // Extract data source transparency info from partner chart
      const partnerDataSource = resolvedPartnerChart?._dataSource || null;

      const partnerName =
        data.partner_name ||
        data.compatibility_data?.partner_name ||
        resolvedPartnerChart?.name ||
        'The Partner';
      ...
      // Get section configuration from orchestrator (synchronous, no AI call)
      const sectionConfig = SectionOrchestrator(chartDataForOrchestrator);

      await progressCallback?.(10, 'Generating all report sections...');

      // Build configs for all independent sections (AI calls run in parallel)
      const sectionConfigs = [
        {
          ...
          data: natalChart,
          hasPartnerAsset: false,
        },
        ...(resolvedPartnerChart ? [{
          type: 'compatibility',
          title: 'Compatibility Analysis',
          ...
          data: {
            ...data.compatibility_data,
            user: natalChart,
            partner: resolvedPartnerChart,
            partner_name: partnerName,
            chartData: data.chartData,
            matrix_scores: data.matrix_scores || data.chartData?.matrix_scores || data.compatibility_data?.matrix_scores,
            composite: data.chartData?.composite || data.compatibility_data?.composite,
          },
          hasPartnerAsset: true,
        }] : []),
        {
          type: 'transit',
          title: 'Extended Transit Forecast',
        ...
        },
        {
          ...
          reportType: sectionConfig.reportType,
          data: {
            ...data.destiny_data,
            natalChart,
            birth_date: chartDataForOrchestrator.birth_date,
          },
          hasPartnerAsset: false,
        },
        ...(resolvedPartnerChart ? [{
          ...
          reportType: 'relationship_matrix',
          data: {
            ...data.matrix_data,
            user: natalChart,
            pair: {
      ...
      const sectionResults = await Promise.all(
        sectionConfigs.map(async (config) => {
          try {
            const sectionData = await generateReportContent(config.reportType, config.data);
            return {
              type: config.type,
              title: config.title,
              content: sectionData,
              summary: extractKeyPoints(sectionData.content, config.type),
              hasPartnerAsset: config.hasPartnerAsset,
              error: null,
            };
          } catch (sectionError) {
            logger.error(`[PDF Generator] Error generating ${config.type} section:`, sectionError.message);
      ...
      await progressCallback?.(70, 'Generating chart visuals...');

      // Generate birth chart SVGs and push all sections with their visuals
      const birthChartResult = sectionResults.find(s => s.type === 'birth_chart');
      if (birthChartResult) {
        let chartImageUrl = null;
        try {
          const { generateBirthChartSVG } = await import('./birth-chart-svg.js');
          const chartSVG = generateBirthChartSVG(natalChart, {
            date: natalChart.birth_date,
            time: natalChart.birth_time,
            location: natalChart.location,
          });
          chartImageUrl = `data:image/svg+xml;base64,${Buffer.from(chartSVG).toString('base64')}`;
        } catch (chartError) {
          logger.error('[PDF Generator] Error generating birth chart image:', chartError);
        }
        sections.push({
          type: 'birth_chart',
          title: 'Birth Chart Analysis',
          content: birthChartResult.content,
          summary: birthChartResult.summary,
          chartImage: chartImageUrl,
        });
      }

      // Partner birth chart SVG (if applicable)
      const compatibilityResult = sectionResults.find(s => s.type === 'compatibility');
      if (resolvedPartnerChart) {
        let partnerChartSvg = null;

        // Separate import from generation — catch each failure granularly
        try {
          const { generateBirthChartSVG } = await import('./birth-chart-svg.js');
          partnerChartSvg = generateBirthChartSVG(resolvedPartnerChart, {
            date: resolvedPartnerChart.birth_date,
            time: resolvedPartnerChart.birth_time,
            location: resolvedPartnerChart.location,
          });
        } catch (partnerChartError) {
          logger.error('[PDF Generator] Error generating partner birth chart SVG:', partnerChartError);
        }

        // Fallback: if SVG generation failed, create a minimal placeholder
        if (!partnerChartSvg) {
          const partnerName = resolvedPartnerChart?.name || 'Partner';
          const partnerSun = resolvedPartnerChart?.sun?.sign || resolvedPartnerChart?.sunSign || '—';
          const partnerMoon = resolvedPartnerChart?.moon?.sign || resolvedPartnerChart?.moonSign || '—';
          partnerChartSvg = `<svg viewBox="0 0 1400 1000" preserveAspectRatio="xMidYMid meet" xmlns="http://www.w3.org/2000/svg" style="background: white; width: 100%; height: auto;">
  <defs>
    <style>
      .ph-text { font-family: 'Georgia', serif; }
      .ph-title { font-size: 28px; font-weight: bold; fill: #1f2937; }
      .ph-sub { font-size: 18px; fill: #6b7280; }
      .ph-disclaimer { font-size: 14px; fill: #9ca3af; font-style: italic; }
    </style>
  </defs>
  <text x="700" y="300" text-anchor="middle" class="ph-text ph-title">${partnerName}</text>
  <text x="700" y="360" text-anchor="middle" class="ph-text ph-sub">Partner Birth Chart</text>
  <text x="700" y="430" text-anchor="middle" class="ph-text ph-sub">Sun: ${partnerSun}  •  Moon: ${partnerMoon}</text>
  <text x="700" y="520" text-anchor="middle" class="ph-text ph-disclaimer">Chart visualization unavailable for this partner data set.</text>
  <text x="700" y="560" text-anchor="middle" class="ph-text ph-disclaimer">The analysis above is based on the birth data you provided.</text>
</svg>`;
        }

        const partnerChartImageUrl = `data:image/svg+xml;base64,${Buffer.from(partnerChartSvg).toString('base64')}`;

        const dataSourceLabel = partnerDataSource?.label || 'Based on birth data you entered';
        const dataSourceDisclaimer =
          'This chart was calculated from birth data you provided for your partner. '
          + 'The placements shown are astrological calculations based on that data and '
          + 'have not been independently verified by your partner.';

        sections.push({
          type: 'partner_birth_chart',
          title: 'Partner Birth Chart',
          content: {
            content: dataSourceDisclaimer,
            dataSource: {
              type: partnerDataSource?.type || 'user_provided',
              label: dataSourceLabel,
            },
          },
          summary: 'Partner birth chart visual',
          chartImage: partnerChartImageUrl,
          chartDataSource: {
            type: partnerDataSource?.type || 'user_provided',
            label: dataSourceLabel,
          },
        ...
        if (compatibilityResult && !compatibilityResult.error) {
          const compSection = {
            type: 'compatibility',
            title: 'Compatibility Analysis',
          ...
          // Generate composite chart SVG from composite data
          try {
            const compositeData = data.chartData?.composite || data.compatibility_data?.composite || data.compatibility_data?.compositeChart;
            if (compositeData) {
              const { generateCompositeChartSVG } = await import('@/src/utils/visuals/generateCompositeChartSVG');
              const svgInput = normalizeCompositeData(compositeData);
              if (svgInput) {
                compSection.compositeChartSvg = generateCompositeChartSVG(svgInput);
              }
            }
          } catch (e) {
            logger.error('[PDF Generator] Error generating composite chart SVG:', e);
          }
      ...
      // Transit forecast
      const transitResult = sectionResults.find(s => s.type === 'transit');
      if (transitResult && !transitResult.error) {
        sections.push({
          type: 'transit',
          title: 'Extended Transit Forecast',
          content: transitResult.content,
          summary: transitResult.summary,
        });
      }

      // Destiny path
      const destinyResult = sectionResults.find(s => s.type === sectionConfig.type);
      if (destinyResult && !destinyResult.error && destinyResult.content && destinyResult.content.content) {
        sections.push({
          type: sectionConfig.type,
          title: sectionConfig.title,
          content: destinyResult.content,
          summary: destinyResult.summary,
        });
      }

      // Relationship matrix (if partner data)
      const matrixResult = sectionResults.find(s => s.type === 'matrix');
      if (matrixResult && !matrixResult.error && matrixResult.content && matrixResult.content.content) {
        ...
        try {
          ...
          logger.error('[PDF Generator] Error generating matrix radar chart:', chartError);
        }

        sections.push({
          type: 'matrix',
          title: 'Relationship Matrix',
          content: matrixResult.content,
          summary: matrixResult.summary,
          chartImage: matrixChartImage,
          matrixChartSVG: matrixChartSVG,
        });
      }

      // Karmic reading
      const karmicResult = sectionResults.find(s => s.type === 'karmic');
      if (karmicResult && !karmicResult.error) {
        sections.push({
          type: 'karmic',
          title: 'Karmic & Shadow Work',
          content: karmicResult.content,
          summary: karmicResult.summary,
        });
      }
    }
    
    // -----------------------------------------------------------------------
    // Deduplicate paragraphs across sections (GSTA-158)
    // Configurable via REPORT_DEDUPLICATION_ENABLED (default: true for MASTER)
    // -----------------------------------------------------------------------
    const deduplicationEnabled = 
      process.env.REPORT_DEDUPLICATION_ENABLED === 'true' ||
      (process.env.REPORT_DEDUPLICATION_ENABLED !== 'false' && reportType === 'MASTER');
    
    if (deduplicationEnabled) {
      await progressCallback?.(85, 'Removing duplicate paragraphs...');
      const duplicateFindings = auditSections(sections, {
        minLength: 60,
        nearThreshold: 0.82,
        maxLengthRatio: 1.5,
      });
      ...
      if (duplicateFindings.length > 0) {
        logger.info(
          `[PDF Generator] Found ${duplicateFindings.length} duplicate/near-duplicate paragraphs:`,
          duplicateFindings.map(f => `${f.type} "${f.preview.substring(0, 50)}..."`).join(', ')
        );
      }
      ...
      sections = deduplicateSections(sections, {
        ...
        nearThreshold: 0.82,
        maxLengthRatio: 1.5,
        skipTypes: ['closing'], // Never deduplicate closing blessing
      });
      
      if (duplicateFindings.length > 0) {
        logger.info(
          `[PDF Generator] Deduplication complete: ${duplicateFindings.length} findings processed`
        );
      }
    }
    ...
    // -----------------------------------------------------------------------
    // Cross-section consistency validation (GSTA-50)
    // -----------------------------------------------------------------------
    const natalChartForValidation = data.natalChart || data.birth_chart_data;
    const partnerChartForValidation =
      data.compatibility_data?.partner ||
      data.chartData?.partner ||
      data.matrix_data?.partner ||
      data.matrix_data?.chart2 ||
    ...
    // Build canonical planet placements from natal chart for cross-section validation
    const canonicalPlanets = natalChartForValidation?.planets || {};

    // Build canonical nodal axis from natal chart for karmic section validation
    const canonicalNodes = {
      northNode: natalChartForValidation?.planets?.northNode || natalChartForValidation?.planets?.['North Node'] || null,
      southNode: natalChartForValidation?.planets?.southNode || natalChartForValidation?.planets?.['South Node'] || null,
    };

    // If planetSignHouseCombinations is available, enrich node house data
    if (natalChartForValidation?.planetSignHouseCombinations) {
      const nnCombo = natalChartForValidation.planetSignHouseCombinations.find(
        c => c.planet === 'North Node' || c.planet === 'True Node' || c.planet?.toLowerCase() === 'north node'
      );
      const snCombo = natalChartForValidation.planetSignHouseCombinations.find(
        c => c.planet === 'South Node' || c.planet?.toLowerCase() === 'south node'
      );
      if (nnCombo) {
        canonicalNodes.northNode = { ...canonicalNodes.northNode, house: nnCombo.house, houseName: nnCombo.houseName };
      }
      if (snCombo) {
        canonicalNodes.southNode = { ...canonicalNodes.southNode, house: snCombo.house, houseName: snCombo.houseName };
      }
    }

    const validationResult = validateReportConsistency({
      sections,
      canonicalNames: {
        userName: data.name || natalChartForValidation?.name || '',
        partnerName:
          data.partner_name ||
          data.compatibility_data?.partner_name ||
          partnerChartForValidation?.name ||
          '',
      },
      canonicalBirthData: {
        birthDate: natalChartForValidation?.birth_date || data.birth_date || '',
        birthTime: natalChartForValidation?.birth_time || data.birth_time || '',
        location: natalChartForValidation?.location || data.location || '',
        partnerBirthDate: partnerChartForValidation?.birth_date || '',
        partnerBirthTime: partnerChartForValidation?.birth_time || '',
        partnerLocation: partnerChartForValidation?.location || '',
      },
      canonicalTransits: data.transit_data?.transits || data.transits || [],
      canonicalPlanets,
      canonicalNodes,
      options: {
        strict: process.env.REPORT_CONSISTENCY_STRICT === 'true',
        allowDuplicateTypes: ['closing'],
      },
    });

    if (!validationResult.valid) {
      logger.error('[PDF Generator] Cross-section consistency errors:', validationResult.errors);
      if (process.env.REPORT_CONSISTENCY_STRICT === 'true') {
        throw new Error(
          `Cross-section consistency validation failed: ${validationResult.errors.join('; ')}`
        );
      }
    }
    if (validationResult.warnings.length > 0) {
      logger.warn('[PDF Generator] Cross-section consistency warnings:', validationResult.warnings);
    }

    // Generate HTML and PDF (skip PDF if skipPdf option is true)
    ...
    let pdfUrl = null;
    if (!data.skipPdf) {
      await progressCallback?.(95, 'Generating premium PDF...');
      try {
        // Use premium PDF generator by default
        const { generatePremiumPdf } = await import('./premium-pdf-generator.js');
        const { uploadPdfToCloudinary } = await import('./pdf-utils.js');
        
        // Extract chart SVGs from sections
      let birthChartSvg = null;
      let partnerBirthChartSvg = null;
      let compatibilityChartSvg = null;
      let compositeChartSvg = null;
        ...
        const partnerBirthChartSection = sections.find(s => s.type === 'partner_birth_chart');
        if (partnerBirthChartSection?.chartImage && partnerBirthChartSection.chartImage.startsWith('data:image/svg+xml')) {
          const base64Match = partnerBirthChartSection.chartImage.match(/base64,(.+)/);
          if (base64Match) {
            try {
              partnerBirthChartSvg = Buffer.from(base64Match[1], 'base64').toString('utf-8');
        ...
        const natalChart = data.natalChart || data.birth_chart_data;
        const userData = {
          name: data.name || 'Beloved Seeker',
          birthDate: natalChart?.birth_date || data.birth_date || '',
          birthTime: natalChart?.birth_time || data.birth_time || '',
          location: natalChart?.location || data.location || '',
          sunSign: natalChart?.sun?.sign || data.sun_sign || '',
          moonSign: natalChart?.moon?.sign || data.moon_sign || '',
          risingSign: natalChart?.rising?.sign || natalChart?.ascendant?.sign || data.rising_sign || '',
        birthChartSvg: birthChartSvg,
        partnerBirthChartSvg: partnerBirthChartSvg,
        compatibilityChartSvg: compatibilityChartSvg,
        compositeChartSvg: compositeChartSvg,
          sections: sections.map(s => ({
            ...
            content: typeof s.content === 'string' ? s.content : (s.content?.content || ''),
            chartImage: s.chartImage || null,
            chartDataSource: s.chartDataSource || null,
          })),
          compatibilityScores: data.matrix_scores || data.chartData?.matrix_scores || data.compatibility_data?.matrix_scores,
        };
        ...
        logger.error('[PDF Generator] Premium PDF generation failed, falling back to standard generator:', premiumError);
        // Fallback to standard PDF generator
        const pdf = await generatePDF(reportType, data, { content: fullContent, sections });
        pdfUrl = pdf.pdfUrl;
      }
    } else {
      await progressCallback?.(95, 'Skipping PDF generation (skipPdf option set)...');
    }
    ...
    logger.error('[PDF Generator] Error generating premium report:', error);
    throw error;
  }
}
