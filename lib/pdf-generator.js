...
export async function generatePremiumReport(reportType, data, progressCallback) {
  try {
    ...
    if (reportType === 'ESSENTIAL') {
      ...
      sections.push({ type: 'tarot', title: 'Tarot Reading', content: tarot });
      ...
      sections.push({ type: 'moon', title: 'Moon Phase Reading', content: moon });
      ...
      sections.push({ type: 'transit', title: 'Short-Term Forecast', content: forecast });
    ...
    const keyThemes = sections.map(s => s.title).join(', ');
    const closing = await generateText(
      getClosingBlessingPrompt({ name: data.name, key_themes: [keyThemes] })
    );
