...
export async function generatePdfFromHtml(html, options = {}) {
  ...
  try {
    ...
    if (executablePath) {
      ...
      // Fallback: let Puppeteer find Chrome/Chromium automatically (for local dev)
      console.warn('[PDF Generator] Could not get chromium executablePath, using system Chrome');
    }
  } catch (error) {
    // If chromium.executablePath() fails (e.g., brotli files not found), fallback to system Chrome
    console.warn('[PDF Generator] Chromium executablePath failed, using system Chrome:', error.message);
