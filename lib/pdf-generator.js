  ...
  try {
    ...
    if (reportType === 'ESSENTIAL') {
      ...
      if (resolvedPartnerChart) {
        let partnerChartImageUrl = null;
        try {
          const { generateBirthChartSVG } = await import('./birth-chart-svg.js');
          const partnerChartSvg = generateBirthChartSVG(resolvedPartnerChart, {
            date: resolvedPartnerChart.birth_date,
            time: resolvedPartnerChart.birth_time,
            location: resolvedPartnerChart.location,
          });
          partnerChartImageUrl = `data:image/svg+xml;base64,${Buffer.from(partnerChartSvg).toString('base64')}`;
        } catch (partnerChartError) {
          logger.error('[PDF Generator] Error generating partner birth chart image:', partnerChartError);
        }
