...
export async function generatePremiumReport(reportType, data, progressCallback) {
  try {
    ...
    if (reportType === 'ESSENTIAL') {
      ...
      // Generate partner birth chart SVG and insert immediately after user birth chart
      let partnerChartImageUrl = null;
      try {
        const { generateBirthChartSVG } = await import('./birth-chart-svg.js');
        const partnerChartSvg = generateBirthChartSVG(
      ...
      if (resolvedPartnerChart) {
        ...
        await progressCallback?.(20, 'Skipping Compatibility analysis (partner chart data unavailable)...');
      }
      ...
      const partnerChart = resolvedPartnerChart || data.matrix_data?.partner || data.matrix_data?.chart2 || data.matrix_data?.pair?.partner;
