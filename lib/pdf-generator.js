...
async function enrichBirthChartData(rawData) {
  ...
  // If still missing, recalculate from raw chart data
  if (!premiumData.planetSignHouseCombinations || !premiumData.houseCuspsDetailed) {
    try {
  ...
  return {
    ...
    planets: rawData.planets || rawData.natal_positions || {},
    houses: rawData.houses || {},
    aspects: rawData.aspects || {},
    sun: rawData.planets?.sun || rawData.natal_positions?.sun || rawData.sun,
    moon: rawData.planets?.moon || rawData.natal_positions?.moon || rawData.moon,
