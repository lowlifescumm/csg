...
export async function monitorAllTransitSubscriptions() {
  logger.info('[Transit Monitor] Starting monitoring cycle...');
  ...
  try {
    ...
    logger.info(`[Transit Monitor] Found ${subscriptions.length} subscriptions to check`);
    ...
    for (const subscription of subscriptions) {
      try {
        ...
        logger.error(`[Transit Monitor] Error checking subscription ${subscription.id}:`, error);
        errors++;
      }
    }

    logger.info(`[Transit Monitor] Cycle complete. Sent ${notificationsSent} notifications. Errors: ${errors}`);
    ...
    logger.error('[Transit Monitor] Fatal error in monitoring cycle:', error);
    return {
      success: false,
      error: error.message
    };
...
async function sendNotifications({
  ...
  if (notifyEmail && email) {
    try {
      ...
      logger.error('[Transit Monitor] Email notification failed:', error);
      await logNotification(
        subscription.user_id,
        transit.id,
        subscription.id,
  ...
  if (notifyPush) {
    try {
      ...
      logger.info('[Transit Monitor] Push notifications not yet implemented');
    } catch (error) {
      logger.error('[Transit Monitor] Push notification failed:', error);
    }
  }
  ...
  if (notifyWebhook && webhookUrl) {
    try {
      ...
      logger.error('[Transit Monitor] Webhook notification failed:', error);
      await logNotification(
        subscription.user_id,
        transit.id,
        subscription.id,
...
export async function cleanupOldNotifications() {
  ...
  logger.info(`[Transit Monitor] Cleaned up ${rowCount} old notification records`);
  return rowCount;
}
