...
export async function sendPasswordResetEmail(email, firstName, resetToken) {
  if (!resend) {
    logger.info('[Email] Resend not configured, would send password reset to:', email);
...
export async function sendPasswordResetConfirmationEmail(email, firstName) {
  if (!resend) {
    logger.info('[Email] Resend not configured, would send password reset confirmation to:', email);
    return { success: false, error: 'RESEND_NOT_CONFIGURED' };
  }
...
export async function sendTransitNotificationEmail(email, firstName, subject, body) {
  if (!resend) {
    logger.info('[Email] Resend not configured, would send transit notification to:', email);
    return { success: false, error: 'RESEND_NOT_CONFIGURED' };
  }
  ...
  try {
    ...
    logger.error('[Email] Failed to send transit notification:', error);
    return { success: false, error: error.message };
  }
}
...
export async function sendNewsletterLeadMagnetEmail(email, firstName) {
  if (!resend) {
    logger.info('[Email] Resend not configured, would send newsletter to:', email);
    return { success: false, error: 'RESEND_NOT_CONFIGURED' };
  }
  ...
  try {
    ...
    logger.error('[Email] Failed to send newsletter:', error);
    return { success: false, error: error.message };
  }
}
