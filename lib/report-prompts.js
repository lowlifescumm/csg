...
Do not perform astrological calculations, reverse-engineer signs from dates, or invent missing chart values.

Use only the chart data explicitly provided in the input and interpret that data.
...
Analyze the relationship between ${user1Name} (${userSunSign}) and ${safePartnerName} (${partnerSunSign}). ${safePartnerName} also has a ${partnerMoonSign} Moon. Reference these specific signs and names throughout your analysis.`
  ...
  const resolvedPartnerSunSign = hasPartnerData && fallbackPartnerSunSign === 'Unknown' ? partnerSunSign : fallbackPartnerSunSign;
  ...
  // Check if partner has complete birth data or just user-provided sign placements
  const hasPartnerBirthData = hasPartnerData && (
    data.partner?.birth_date || data.partner?.birthDate ||
    chartData?.partner?.birth_date || chartData?.partner?.birthDate ||
    data.partner?.birth_time || data.partner?.birthTime
  );
  const partnerDisclaimer = hasPartnerData
    ? (hasPartnerBirthData
      ? '\n\n*Partner analysis based on complete birth chart data provided by user.*'
      : '\n\n*Partner analysis based on birth data provided by user; chart available upon request. Partner birth time and location were not provided, so this analysis is based on sign-level placements.*')
    : '';
...
- User 2 (Partner): **${safePartnerName || 'The Partner'}** - ${resolvedPartnerSunSign} Sun
...
${(() => {
    if (!compositeChartData) return '';
    const sunHouseSuffix = compositeChartData.sun.house === 1 ? 'st' : compositeChartData.sun.house === 2 ? 'nd' : compositeChartData.sun.house === 3 ? 'rd' : 'th';
    const moonHouseSuffix = compositeChartData.moon.house === 1 ? 'st' : compositeChartData.moon.house === 2 ? 'nd' : compositeChartData.moon.house === 3 ? 'rd' : 'th';
    return `
**CRITICAL - Composite Chart Data Provided:**
The Composite Chart features a Sun in **${compositeChartData.sun.sign}** (${compositeChartData.sun.house}${sunHouseSuffix} House) and a Moon in **${compositeChartData.moon.sign}** (${compositeChartData.moon.house}${moonHouseSuffix} House). The Composite Rising is **${compositeChartData.rising.sign}**.
...
4. **Long-Term Potential:** ${(() => {
    if (compositeChartData) {
      const sunHouseSuffix = compositeChartData.sun.house === 1 ? 'st' : compositeChartData.sun.house === 2 ? 'nd' : compositeChartData.sun.house === 3 ? 'rd' : 'th';
      const moonHouseSuffix = compositeChartData.moon.house === 1 ? 'st' : compositeChartData.moon.house === 2 ? 'nd' : compositeChartData.moon.house === 3 ? 'rd' : 'th';
      return `The Composite Chart features a Sun in ${compositeChartData.sun.sign} (${compositeChartData.sun.house}${sunHouseSuffix} House) and a Moon in ${compositeChartData.moon.sign} (${compositeChartData.moon.house}${moonHouseSuffix} House). Use these placements to describe the relationship's ultimate purpose and destiny.`;
    }
    return 'Interpret the Composite Sun and Moon house placements to describe the relationship\'s ultimate purpose. What is this relationship meant to teach both people? What is its karmic or spiritual purpose?';
  })()}
...
export function getShortTransitPrompt(data) {
  ...
  const futureTransits = transitList.filter(t => {
    ...
    if (date.getFullYear() < currentYear) {
      logger.warn(`[Short Transit Filter] Rejecting past date: ${transitDate} (year ${date.getFullYear()} < ${currentYear})`);
      return false;
    }
...
export function getExtendedTransitPrompt(data) {
  ...
  const futureTransits = (transits || []).filter(t => {
    ...
    // CRITICAL: Reject any dates from previous years (fixes 2023 dates appearing)
    if (date.getFullYear() < currentYear) {
      logger.warn(`[Extended Transit Filter] Rejecting past date: ${transitDate} (year ${date.getFullYear()} < ${currentYear})`);
      return false;
    }
    return date >= today;
  }).sort((a, b) => {
...
${transitDatesList || '(No specific transit data available for this period.)'}
...
5. **No Missing Data References:** If no transit data is shown above, do NOT reference the absence of data. Do NOT say "no transits" or "no data" or any variation. Simply provide a general astrological forecast based on the birth chart, focusing on current planetary positions and themes.
...
export function getRelationshipMatrixPrompt(data) {
  ...
  const hasPartnerBirthData = partnerChart?.birth_date || partnerChart?.birthDate || partnerChart?.birth_time || partnerChart?.birthTime;
  const partnerDisclaimer = hasPartnerBirthData
    ? '\n\n*Partner analysis based on complete birth chart data provided by user.*'
    : '\n\n*Partner analysis based on birth data provided by user. Partner birth time and location were not provided, so this analysis is based on sign-level placements.*';
...
/**
 * Extract ALL hard aspects from chart data (Square, Opposition, tight Conjunction)
 * for use in shadow work analysis. Unlike the existing Node-only filter, this captures
 * every challenging inter-planetary dynamic in the chart.
 *
 * Returns structured data grouped by aspect type with orb context and house info.
 */
function getHardAspects(data) {
  const aspects = data?.aspects || data?.natalChart?.aspects || [];
  const planetSignHouseCombinations = data?.planetSignHouseCombinations ||
    data?.natalChart?.planetSignHouseCombinations || data?.birth_chart_data?.planetSignHouseCombinations || [];

  if (!Array.isArray(aspects) || aspects.length === 0) {
    return { squares: [], oppositions: [], tightConjunctions: [], hasHardAspects: false, summary: 'No hard aspects identified' };
  }

  const hardAspectTypes = ['Square', 'Opposition'];

  function getPlanetHouse(planetName) {
    if (!Array.isArray(planetSignHouseCombinations)) return null;
    const combo = planetSignHouseCombinations.find(c =>
      c.planet === planetName || c.planet?.toLowerCase() === planetName?.toLowerCase()
    );
    return combo ? { house: combo.house, houseName: combo.houseName || combo.sign || '' } : null;
  }

  function formatAspect(a) {
    const planet1 = a.planet1 || a.planet || 'Unknown';
    const planet2 = a.planet2 || a.to || 'Unknown';
    const aspectType = a.type || a.aspect || 'aspect';
    const orb = typeof a.orb === 'number' ? a.orb : null;
    const house1 = getPlanetHouse(planet1);
    const house2 = getPlanetHouse(planet2);
    ...
    return {
      ...
      type: aspectType,
      orb,
      planet1House: house1?.house || null,
      planet1HouseName: house1?.houseName || null,
      planet2House: house2?.house || null,
      planet2HouseName: house2?.houseName || null,
      description: orb !== null
        ? `${planet1} ${aspectType} ${planet2} (Orb ${orb.toFixed(1)}°)`
  ...
  const squares = [];
  const oppositions = [];
  const tightConjunctions = [];

  for (const a of aspects) {
    const aspectType = a.type || a.aspect || '';
    const typeLower = aspectType.toLowerCase();

    if (typeLower === 'square' || aspectType === 'Square') {
      squares.push(formatAspect(a));
    } else if (typeLower === 'opposition' || aspectType === 'Opposition') {
      oppositions.push(formatAspect(a));
    } else if ((typeLower === 'conjunction' || aspectType === 'Conjunction') && typeof a.orb === 'number' && a.orb < 3) {
      tightConjunctions.push(formatAspect(a));
    }
  }

  const hasHardAspects = squares.length > 0 || oppositions.length > 0 || tightConjunctions.length > 0;

  let summary = 'No hard aspects identified';
  if (hasHardAspects) {
    const parts = [];
    if (squares.length > 0) parts.push(`${squares.length} Square${squares.length > 1 ? 's' : ''}`);
    if (oppositions.length > 0) parts.push(`${oppositions.length} Opposition${oppositions.length > 1 ? 's' : ''}`);
    if (tightConjunctions.length > 0) parts.push(`${tightConjunctions.length} tight Conjunction${tightConjunctions.length > 1 ? 's' : ''}`);
    summary = parts.join(', ');
  }

  return { squares, oppositions, tightConjunctions, hasHardAspects, summary };
}

/**
 * Build the hard aspect analysis text block for the prompt.
 */
function buildHardAspectAnalysisText(data) {
  const { squares, oppositions, tightConjunctions, hasHardAspects, summary } = getHardAspects(data);

  if (!hasHardAspects) {
    return '';
  }

  let text = '\n**Full Hard Aspect Analysis (Chart-Wide):**\n\n';
  text += `The following ${summary} create shadow tension patterns across your chart:\n\n`;

  if (squares.length > 0) {
    text += '**Squares (Internal Tension & Growth Edges):**\n';
    squares.forEach(a => {
      text += `- ${a.description}`;
      if (a.planet1HouseName) text += ` — ${a.planet1} in the ${a.planet1HouseName}`;
      if (a.planet2HouseName) text += `, ${a.planet2} in the ${a.planet2HouseName}`;
      text += '\n';
    });
    text += '\n';
  }
  ...
  if (oppositions.length > 0) {
    text += '**Oppositions (Polarity & Relationship Dynamics):**\n';
    oppositions.forEach(a => {
      text += `- ${a.description}`;
      if (a.planet1HouseName) text += ` — ${a.planet1} in the ${a.planet1HouseName}`;
      if (a.planet2HouseName) text += `, ${a.planet2} in the ${a.planet2HouseName}`;
      text += '\n';
  ...
  if (tightConjunctions.length > 0) {
    text += '**Tight Conjunctions (Intensified Archetypal Fusion):**\n';
    tightConjunctions.forEach(a => {
      text += `- ${a.description}`;
      if (a.planet1HouseName) text += ` — ${a.planet1} in the ${a.planet1HouseName}`;
      if (a.planet2HouseName) text += `, ${a.planet2} in the ${a.planet2HouseName}`;
      text += '\n';
...
export function getKarmicReadingPrompt(data) {
  ...
  // Build the full hard aspect analysis block
  const hardAspectBlock = buildHardAspectAnalysisText(data);
  const hasHardAspects = hardAspectBlock.length > 0;

  // Build the chart-wide hard aspects context block that lists ALL challenging aspects
  let chartWideHardAspectList = '';
  if (hasHardAspects) {
    const { squares, oppositions, tightConjunctions } = getHardAspects(data);
    const lines = [];
    squares.forEach(a => lines.push(`- ${a.description}`));
    oppositions.forEach(a => lines.push(`- ${a.description}`));
    tightConjunctions.forEach(a => lines.push(`- ${a.description}`));
    chartWideHardAspectList = lines.join('\n');
  }

  return `${SYSTEM_PROMPT}

Generate a premium Karmic and Shadow Work reading. The analysis MUST be built around the House placements of the Nodal Axis AND the chart-wide hard aspects (Squares, Oppositions, tight Conjunctions) that create shadow dynamics.${hardAspectBlock}
...
**Karmic Challenge Aspects (Node-Related):**

- ${challengingAspectsList}
${hasHardAspects ? `

**Chart-Wide Hard Aspects (Shadow Tensions):**

${chartWideHardAspectList}

These aspects create shadow dynamics that must be addressed in addition to nodal patterns.` : ''}
...
2. **Emotional Patterns to Break:** Directly interpret ALL challenging aspects — both the Node-related challenges AND the chart-wide hard aspects. For each aspect, explain the unconscious pattern, how it manifests in daily life, and what must be released or transformed.

3. **Practical Shadow Work Guidance:** Provide 3-5 specific, personalized shadow work exercises that address the themes of the South Node House, the challenging aspects to the Nodes, AND the chart-wide hard aspects.

4. **Shadow Patterns from Hard Aspects:** Analyze how the chart-wide hard aspects create shadow patterns. For each hard aspect, explain the unconscious tension, the shadow manifestation, and the integration path. Connect these patterns to the user's Nodal Axis journey.
...
Directly interpret each challenging aspect — both Node-related AND chart-wide hard aspects. Provide comprehensive analysis of what emotional patterns or behaviors these aspects indicate must be released or transformed. Be specific about how each aspect manifests. Give detailed guidance on recognizing and working with these patterns.
${hasHardAspects ? `
**Hard Aspect Shadow Patterns**
Analyze each hard aspect in the chart. For each aspect, provide:
- The unconscious dynamic created by this tension
- How it manifests as a shadow pattern in daily life
- Which specific behaviors, defenses, or coping mechanisms arise from it
- How this pattern connects to the user's Nodal Axis journey
- A practical path for integrating this shadow pattern` : ''}
...
- Address the challenging aspects to the Nodes${hasHardAspects ? '\n- Address the chart-wide hard aspects and their shadow patterns' : ''}
...
Write with depth, specificity, and warmth. Every interpretation must reference the specific house placements and explain how the karmic lessons manifest in those life areas. Additionally, every chart-wide hard aspect must be interpreted for its shadow dynamic, not just the Node-related aspects.`;
}
...
/**
 * Advanced Report — Detailed House Placements for All Planets
 */
export function getAdvancedHousesPrompt(data) {
  const { name, natalChart } = data;
  const combinations = natalChart?.planetSignHouseCombinations || [];

  const allPlacements = combinations.length > 0
    ? combinations.map(c => `- ${c.planet} in ${c.sign} in the ${c.houseName || formatOrdinal(c.house)}${c.degree !== undefined ? ` at ${c.degree}°` : ''}`).join('\n')
    : 'No detailed house placements provided.';

  return `${SYSTEM_PROMPT}

Generate a detailed, personalized analysis of EVERY planet's house placement in the user's birth chart.

User: ${name}

**Complete Planet-House Placements:**
${allPlacements}

**Instruction:**

For EACH planet listed above, provide:
1. **What the planet represents** in astrology (its core domain)
2. **How it expresses** through its sign (the *how*)
3. **Where it manifests** through its house (the *where* in life)
4. **One specific, personalized action step** for working with this energy

Be thorough and cover ALL planets provided — do not skip any. Write in narrative form, not bullet points. Integrate sign and house into every interpretation. Keep the tone warm and spiritually insightful.`;
}

/**
 * Advanced Report — Aspect Interpretations
 */
export function getAdvancedAspectsPrompt(data) {
  const { name, natalChart } = data;
  const aspects = natalChart?.aspects || data?.aspects || [];

  const majorAspects = aspects.filter(a =>
    ['Conjunction', 'Opposition', 'Trine', 'Square', 'Sextile'].includes(a.type)
  );

  const aspectList = majorAspects.length > 0
    ? majorAspects.map(a =>
        `- ${a.planet1 || a.planet_1} ${a.type} ${a.planet2 || a.planet_2}${a.orb ? ` (orb ${a.orb.toFixed(1)}°)` : ''}`
      ).join('\n')
    : 'No major aspects provided.';

  return `${SYSTEM_PROMPT}

Generate a narrative interpretation of each major natal aspect for the user.
...
For EACH major aspect listed above, write a paragraph (or two) that explains:
1. **The psychological tension or harmony** this aspect creates
2. **How it manifests** in the person's daily life and relationships
3. **A concrete example** of how this aspect might show up
4. **One empowering strategy** for working with this energy

Cover every aspect listed — do not skip any. Write in flowing narrative, not bullets. Be specific to the planets and aspect type involved.`;
}

/**
 * Advanced Report — Career Path Analysis (10th House / MC)
 */
export function getAdvancedCareerPrompt(data) {
  const { name, natalChart } = data;
  const combinations = natalChart?.planetSignHouseCombinations || [];
  const mcCombo = combinations.find(c => c.house === 10 || c.houseName?.includes('Midheaven'));
  const tenthHousePlanets = combinations.filter(c => c.house === 10);
  const saturnCombo = combinations.find(c => c.planet === 'Saturn');
  const marsCombo = combinations.find(c => c.planet === 'Mars');
  const sunCombo = combinations.find(c => c.planet === 'Sun');

  const mcInfo = mcCombo
    ? `Midheaven / 10th House Cusp: ${mcCombo.sign}`
    : (natalChart?.houses?.[10]?.sign ? `10th House Cusp: ${natalChart.houses[10].sign}` : '10th House data not provided');

  const planetsInTenth = tenthHousePlanets.length > 0
    ? tenthHousePlanets.map(c => `- ${c.planet} in ${c.sign}${c.degree !== undefined ? ` at ${c.degree}°` : ''}`).join('\n')
    : 'No planets in the 10th House.';

  return `${SYSTEM_PROMPT}

Generate a focused career and vocational analysis based on the user's 10th House (Midheaven) and relevant planets.
...
${planetsInTenth}
- Saturn (structure / discipline): ${saturnCombo ? `${saturnCombo.sign} in the ${saturnCombo.houseName || formatOrdinal(saturnCombo.house)}` : 'Not available'}
- Mars (drive / ambition): ${marsCombo ? `${marsCombo.sign} in the ${marsCombo.houseName || formatOrdinal(marsCombo.house)}` : 'Not available'}
- Sun (core identity): ${sunCombo ? `${sunCombo.sign} in the ${sunCombo.houseName || formatOrdinal(sunCombo.house)}` : 'Not available'}

**Instruction:**
1. **Interpret the Midheaven sign** — what style of public life and career suits this person?
2. **Analyze each 10th House planet** — how does it color their professional expression?
3. **Synthesize Saturn, Mars, and Sun** — how do these planets shape their work ethic, ambition, and leadership?
4. **Provide 3 specific career directions or strategies** aligned with these placements
5. **Warn about 1-2 potential career pitfalls** indicated by the chart

Write in warm, specific narrative. Do not ask for missing data — interpret what is provided.`;
}

/**
 * Advanced Report — Relationship Insights (Solo Perspective)
 */
export function getAdvancedRelationshipsPrompt(data) {
  const { name, natalChart } = data;
  const combinations = natalChart?.planetSignHouseCombinations || [];
  const venusCombo = combinations.find(c => c.planet === 'Venus');
  const marsCombo = combinations.find(c => c.planet === 'Mars');
  const moonCombo = combinations.find(c => c.planet === 'Moon');
  const seventhHouse = combinations.filter(c => c.house === 7);
  const fifthHouse = combinations.filter(c => c.house === 5);

  return `${SYSTEM_PROMPT}

Generate a relationship analysis from the user's SOLO natal chart perspective — what they bring to partnerships, not synastry with a partner.

User: ${name}

**Relationship Data:**
- Venus (love style): ${venusCombo ? `${venusCombo.sign} in the ${venusCombo.houseName || formatOrdinal(venusCombo.house)}` : 'Not available'}
- Mars (desire / pursuit): ${marsCombo ? `${marsCombo.sign} in the ${marsCombo.houseName || formatOrdinal(marsCombo.house)}` : 'Not available'}
- Moon (emotional needs): ${moonCombo ? `${moonCombo.sign} in the ${moonCombo.houseName || formatOrdinal(moonCombo.house)}` : 'Not available'}
- Planets in the 7th House (partnerships): ${seventhHouse.length > 0 ? seventhHouse.map(c => `${c.planet} in ${c.sign}`).join(', ') : 'None provided'}
- Planets in the 5th House (romance): ${fifthHouse.length > 0 ? fifthHouse.map(c => `${c.planet} in ${c.sign}`).join(', ') : 'None provided'}

**Instruction:**
1. **Love Style (Venus):** How do they express affection? What do they value in a partner?
2. **Desire & Pursuit (Mars):** How do they initiate romance? What turns them on?
3. **Emotional Needs (Moon):** What do they need to feel safe and loved?
4. **Partnership Patterns (7th House):** What kind of partner do they attract? What karmic relationship themes repeat?
5. **Romance & Play (5th House):** How do they date and have fun in love?
6. **Actionable Advice:** 3 specific practices to improve their relationship life

Write in warm narrative. Focus on self-awareness, not partner-blame.`;
}

/**
 * Advanced Report — Life Purpose / North Node Analysis
 */
export function getAdvancedLifePurposePrompt(data) {
  const { name, natalChart } = data;
  const combinations = natalChart?.planetSignHouseCombinations || [];
  const northNodeCombo = combinations.find(c => c.planet === 'North Node');
  const southNodeCombo = combinations.find(c => c.planet === 'South Node');

  // Fallback: check nodes object
  const nodes = natalChart?.nodes || data?.nodes || {};
  const northNodeSign = northNodeCombo?.sign || nodes?.north_node || nodes?.northNode || 'Not available';
  const northNodeHouse = northNodeCombo?.house || nodes?.north_node_house || 'Not available';
  const southNodeSign = southNodeCombo?.sign || nodes?.south_node || nodes?.southNode || 'Not available';
  const southNodeHouse = southNodeCombo?.house || nodes?.south_node_house || 'Not available';

  return `${SYSTEM_PROMPT}

Generate a life purpose and soul-growth analysis centered on the Lunar Nodal Axis.

User: ${name}

**Nodal Axis Data:**
- North Node (soul growth direction): ${northNodeSign}${typeof northNodeHouse === 'number' ? ` in the ${formatOrdinal(northNodeHouse)} House` : (northNodeCombo?.houseName ? ` in the ${northNodeCombo.houseName}` : '')}
- South Node (past-life comfort / patterns to release): ${southNodeSign}${typeof southNodeHouse === 'number' ? ` in the ${formatOrdinal(southNodeHouse)} House` : (southNodeCombo?.houseName ? ` in the ${southNodeCombo.houseName}` : '')}

**Instruction:**
1. **North Node Interpretation:** What is their soul's calling in this lifetime? What skills and experiences must they cultivate?
2. **South Node Shadow:** What old patterns, comfort zones, or past-life habits hold them back?
3. **House Axis:** How do the house placements of the Nodes reveal the ARENA of their karmic work?
4. **Growth Direction:** Provide 3 concrete, daily-life practices that align them with their North Node
5. **Release Practices:** What should they consciously let go of or delegate?

Write with spiritual depth and practical clarity. Help them see their life purpose as an ongoing journey, not a destination.`;
}

/**
 * Advanced Report — Financial Outlook (2nd House, Jupiter, Saturn)
 */
export function getAdvancedFinancialPrompt(data) {
  const { name, natalChart } = data;
  const combinations = natalChart?.planetSignHouseCombinations || [];
  const secondHouse = combinations.filter(c => c.house === 2);
  const jupiterCombo = combinations.find(c => c.planet === 'Jupiter');
  const saturnCombo = combinations.find(c => c.planet === 'Saturn');
  const venusCombo = combinations.find(c => c.planet === 'Venus');

  const secondHouseCusp = natalChart?.houses?.[2]?.sign || (secondHouse[0]?.sign) || 'Not available';

  return `${SYSTEM_PROMPT}

Generate a financial outlook analysis based on the user's 2nd House and wealth indicators.

User: ${name}

**Financial Data:**
- 2nd House Cusp (personal resources): ${secondHouseCusp}
- Planets in the 2nd House: ${secondHouse.length > 0 ? secondHouse.map(c => `${c.planet} in ${c.sign}`).join(', ') : 'None provided'}
- Jupiter (expansion / abundance): ${jupiterCombo ? `${jupiterCombo.sign} in the ${jupiterCombo.houseName || formatOrdinal(jupiterCombo.house)}` : 'Not available'}
- Saturn (restriction / discipline): ${saturnCombo ? `${saturnCombo.sign} in the ${saturnCombo.houseName || formatOrdinal(saturnCombo.house)}` : 'Not available'}
- Venus (value / pleasure): ${venusCombo ? `${venusCombo.sign} in the ${venusCombo.houseName || formatOrdinal(venusCombo.house)}` : 'Not available'}

**Instruction:**
1. **Money Mindset (2nd House Cusp):** What is their core psychological relationship with money and self-worth?
2. **2nd House Planets:** How do any planets in the 2nd House amplify or complicate their financial life?
3. **Jupiter's Abundance:** Where and how does expansion flow for them? What are their natural wealth channels?
4. **Saturn's Structure:** Where must they build discipline, patience, or long-term strategy around resources?
5. **Venus & Values:** What do they truly value, and how does that shape their spending and earning?
6. **Actionable Wealth Strategy:** 3 specific financial practices or mindset shifts aligned with their chart

Write with practical spirituality — grounded, empowering, and specific.`;
}

/**
 * Advanced Report — Health & Wellness (6th House)
 */
export function getAdvancedHealthPrompt(data) {
  const { name, natalChart } = data;
  const combinations = natalChart?.planetSignHouseCombinations || [];
  const sixthHouse = combinations.filter(c => c.house === 6);
  const marsCombo = combinations.find(c => c.planet === 'Mars');
  const saturnCombo = combinations.find(c => c.planet === 'Saturn');
  const moonCombo = combinations.find(c => c.planet === 'Moon');

  const sixthHouseCusp = natalChart?.houses?.[6]?.sign || (sixthHouse[0]?.sign) || 'Not available';

  return `${SYSTEM_PROMPT}

Generate a health and wellness analysis based on the user's 6th House and key planetary indicators.

User: ${name}

**Health Data:**
- 6th House Cusp (daily health habits): ${sixthHouseCusp}
- Planets in the 6th House: ${sixthHouse.length > 0 ? sixthHouse.map(c => `${c.planet} in ${c.sign}`).join(', ') : 'None provided'}
- Mars (energy / vitality): ${marsCombo ? `${marsCombo.sign} in the ${marsCombo.houseName || formatOrdinal(marsCombo.house)}` : 'Not available'}
- Saturn (chronic patterns / structure): ${saturnCombo ? `${saturnCombo.sign} in the ${saturnCombo.houseName || formatOrdinal(saturnCombo.house)}` : 'Not available'}
- Moon (emotional body / rhythms): ${moonCombo ? `${moonCombo.sign} in the ${moonCombo.houseName || formatOrdinal(moonCombo.house)}` : 'Not available'}

**Instruction:**
1. **Daily Health Style (6th House Cusp):** What kind of daily routine and wellness approach suits their constitution?
2. **6th House Planets:** How do any planets here amplify health strengths or sensitivities?
3. **Mars & Vitality:** What fuels their physical energy? What depletes it?
4. **Saturn & Chronic Patterns:** Where might they experience slowness, restriction, or need for preventive care?
5. **Moon & Emotional Body:** How do stress and emotions manifest physically? What self-care rhythms restore them?
6. **Holistic Wellness Plan:** 3 specific, personalized health practices (diet, movement, mindfulness) based on their chart

Include this disclaimer: "This is a spiritual wellness reading based on astrological symbolism. It is not medical advice. Please consult healthcare professionals for medical concerns."

Write with nurturing warmth and practical guidance.`;
}
...
export function getPromptByType(reportType, data) {
  const prompts = {
    ...
    // Advanced report deep analysis sections
    'advanced_houses': getAdvancedHousesPrompt,
    'advanced_aspects': getAdvancedAspectsPrompt,
    'advanced_career': getAdvancedCareerPrompt,
    'advanced_relationships': getAdvancedRelationshipsPrompt,
    'advanced_life_purpose': getAdvancedLifePurposePrompt,
    'advanced_financial': getAdvancedFinancialPrompt,
    'advanced_health': getAdvancedHealthPrompt,
  };
