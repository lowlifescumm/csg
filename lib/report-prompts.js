...
export function getShortTransitPrompt(data) {
  ...
  // Filter transits to only include future dates
  ...
  // Format transit dates for injection
  let transitDatesList = '';
  if (futureTransits.length > 0) {
    transitDatesList = futureTransits.map((transit, idx) => {
...
**Focus on the next 7-14 days from ${todayString}.**
...
Transits Data:
${JSON.stringify(futureTransits, null, 2)}

For each transit:
- Explain what the energy means
- How it affects the user
- Advice for working with it
- Use ONLY the dates provided above - DO NOT invent or reference dates before ${todayString}
