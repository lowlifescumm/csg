...
export function getCompatibilityPrompt(data) {
  ...
  const user2Name = partner_name || (partner?.name || chartData?.partner?.name || 'User 2 (Partner)');
  
  // Normalize chart references for sun/moon extraction
  const normalizedUserChart = user || chartData || {};
  const normalizedPartnerChart = partner || chartData?.partner || {};
  
  const userSunSign = 
    normalizedUserChart?.sun?.sign ||
    normalizedUserChart?.planets?.sun?.sign ||
    normalizedUserChart?.natal_positions?.sun?.sign ||
    'Unknown';
    
  const userMoonSign = 
    normalizedUserChart?.moon?.sign ||
    normalizedUserChart?.planets?.moon?.sign ||
    normalizedUserChart?.natal_positions?.moon?.sign ||
    'Unknown';
    
  const partnerSunSign = 
    normalizedPartnerChart?.sun?.sign ||
    normalizedPartnerChart?.planets?.sun?.sign ||
    normalizedPartnerChart?.natal_positions?.sun?.sign ||
    'Unknown';
    
  const partnerMoonSign = 
    normalizedPartnerChart?.moon?.sign ||
    normalizedPartnerChart?.planets?.moon?.sign ||
    normalizedPartnerChart?.natal_positions?.moon?.sign ||
    'Unknown';
  
  // Build partner chemistry context if chart data is available
  const hasPartnerData = partner || chartData?.partner;
  const chemistryContext = hasPartnerData
    ? `\n\nThe Partner is a ${partnerSunSign} Sun and ${partnerMoonSign} Moon. Analyze the chemistry between the User (${userSunSign}) and Partner (${partnerSunSign}) through these elemental signatures.`
    : '';
  ...
  const rawBreakdown =
    ...
    (typeof matrix_scores === 'object' ? matrix_scores : null);
  
  const compatibilityBreakdown = rawBreakdown && Object.keys(rawBreakdown).length > 0
...
User 1 (Primary): ${fallbackUserSunSign}
User 2 (Partner): ${fallbackPartnerSunSign}${chemistryContext}${compatibilityBreakdown}
...
export function getRelationshipMatrixPrompt(data) {
  ...
  const userChart = pair?.user || data.user || data.chart1 || chartData;
...
The Partner is a ${partnerSunSign} Sun and ${partnerMoonSign} Moon. Analyze the chemistry between the User (${userSunSign}) and Partner (${partnerSunSign}) through these dynamics.
...
3. **For each category**, interpret the score and explain:
...
4. **Tone**: Write with warmth and constructive guidance. Even lower scores represent opportunities for growth, not failures.

Remember: User's Sun sign is ${userSunSign}, Partner's Sun sign is ${partnerSunSign}. Use the calculated scores above to inform your analysis.`;
}
