...
export function getCompatibilityPrompt(data) {
  ...
  const partnerName = hasPartnerData 
    ? (partner?.name || chartData?.partner?.name || data.partner?.name || 'Partner')
    : null;
...
User 2 (Partner): ${fallbackPartnerSunSign}${chemistryContext}${partnerContext}${compatibilityBreakdown}
