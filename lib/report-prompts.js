...
export function getTarotReadingPrompt(data) {
  ...
  // Format the 3-card spread for injection
  let cardSpreadText = '';
...
export function getMoonReadingPrompt(data) {
  ...
  const currentMoonSign = data.moon_sign || 'Unknown'; // Current transit moon sign
  const natalMoonSign = data.natal_moon_sign || data.moon_sign || 'Unknown'; // User's natal moon sign
  const sunSign = data.sun_sign || data.user?.sunSign || data.birth_chart_data?.planets?.sun?.sign || 'Unknown';
