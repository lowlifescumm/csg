...
export async function inlineImage(imageUrl, maxSizeMB = 5) {
  ...
  try {
    ...
    if (!response.ok) {
      console.warn(`[Image Inliner] Failed to fetch image: ${imageUrl} (${response.status})`);
    ...
    if (!contentType.startsWith('image/')) {
      console.warn(`[Image Inliner] URL does not return an image: ${imageUrl} (${contentType})`);
    ...
    if (contentLength) {
      ...
      if (sizeMB > maxSizeMB) {
        console.warn(`[Image Inliner] Image too large: ${imageUrl} (${sizeMB.toFixed(2)}MB, max: ${maxSizeMB}MB)`);
        return imageUrl; // Return original URL
      }
    }
    ...
    if (sizeMB > maxSizeMB) {
      console.warn(`[Image Inliner] Image too large after fetch: ${imageUrl} (${sizeMB.toFixed(2)}MB, max: ${maxSizeMB}MB)`);
      return imageUrl; // Return original URL
    }
    ...
    console.log(`[Image Inliner] Inlined image: ${imageUrl} (${sizeMB.toFixed(2)}MB)`);
    
    return dataUri;
  } catch (error) {
    console.error(`[Image Inliner] Error inlining image ${imageUrl}:`, error.message);
