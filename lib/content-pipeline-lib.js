...
/**
 * Gets the next planned content item from the calendar DB.
 * Falls back to file-based loadBriefs() if DB is unreachable or empty.
 * @returns {Promise<Array>} Array of brief objects (single item from calendar, all from file fallback)
 */
async function getCalendarBrief() {
  try {
    ...
    const { rows } = await pool.query(`
      SELECT
        ...
        cc.notes,
        cb.search_intent,
        cb.target_word_count,
        cb.outline,
        cb.key_points,
        cb.secondary_keywords,
        cb.cta_strategy
      FROM content_calendar cc
      LEFT JOIN content_briefs cb ON cb.calendar_id = cc.id
      WHERE cc.status = 'planned'
    ...
    if (rows.length > 0) {
      ...
      const brief = {
        ...
        title_options: [r.title],
        meta_description: r.notes || `Complete guide to ${r.target_keyword || r.title}`,
        search_intent: r.search_intent || 'informational',
        target_word_count: r.target_word_count || 1500,
        h2_headings: Array.isArray(r.outline)
          ? r.outline
          : r.key_points || ['Introduction', 'Key Concepts', 'Deep Dive', 'Practical Applications', 'FAQ'],
        cta: r.cta_strategy || `Explore our guide to ${r.target_keyword || r.title}`,
        internal_linking: r.key_points || [],
        calendar_id: r.id,
        publish_date: r.publish_date,
        target_url_slug: r.target_url_slug,
      };

      logger.info(`[content-pipeline-lib] Got next calendar item: #${r.id} "${r.title}"`);
      return [brief];
    }

    logger.info('[content-pipeline-lib] No planned items in calendar — falling back to file-based briefs');
  } catch (err) {
    logger.warn('[content-pipeline-lib] Calendar DB unavailable — falling back to file-based briefs:', err.message);
  }

  return loadBriefs();
}

module.exports = { loadBriefs, getCalendarBrief, getThemeForPost, buildImageUrl, generateSocialCopy, slugify, generateArticleContent };
