...
export function flattenReportData(reportData) {
  ...
  if (reportData.sections && Array.isArray(reportData.sections)) {
    reportData.sections.forEach((section, index) => {
      ...
      flattened[contentKey] = section.content?.content || section.content || '';
      
      // Preserve SVG strings from sections (e.g., matrixChartSVG, chartSvg)
      if (section.matrixChartSVG) {
        flattened.matrixChartSVG = section.matrixChartSVG;
      }
      if (section.chartSvg) {
        flattened.chartSvg = section.chartSvg;
      }
      if (section.chartSVG) {
        flattened.chartSVG = section.chartSVG;
      }
...
export async function renderFromTemplate(templateJson, data = {}, options = {}) {
  ...
  if (template.html) {
    ...
    if (options.inlineImages !== false) {
      const { inlineAllImages } = await import('./image-inliner.js');
      rendered = await inlineAllImages(rendered);
    }
