...
import { inlineImagesInHtml } from './image-inliner.js';
        ...
        if (section.chartImage.startsWith('data:image/svg+xml')) {
          ...
          if (base64Match) {
            try {
              ...
              logger.warn('[Template Renderer] Could not decode chart SVG from data URL');
            }
          }
        }
      }
...
export async function generatePDFFromTemplate(template, reportData, options = {}) {
  try {
    ...
    // Generate PDF
    logger.info('[Template Renderer] Generating PDF with pdfme...');
    logger.info('[Template Renderer] Template pages:', templateArray.length);
    logger.info('[Template Renderer] Input fields:', Object.keys(inputData).length);
    ...
    logger.info('[Template Renderer] PDF generated successfully, size:', pdfBuffer.length, 'bytes');
    
    return pdfBuffer;
  } catch (error) {
    logger.error('[Template Renderer] Error generating PDF:', error);
...
export async function generateAndUploadPDF(template, reportData, reportType, options = {}) {
  ...
  const uploadResult = await new Promise((resolve, reject) => {
    const uploadStream = cloudinary.uploader.upload_stream(
      ...
      (error, result) => {
        if (error) {
          logger.error('[Template Renderer] Cloudinary upload error:', error);
          reject(error);
        } else {
          resolve(result);
        }
  ...
  logger.info('[Template Renderer] PDF uploaded successfully:', uploadResult.secure_url);
...
export async function renderTemplatePDF(templateId, reportData, reportType, options = {}) {
  ...
  logger.info(`[Template Renderer] Using template: ${template.name} (ID: ${template.id})`);
...
export async function renderFromTemplate(templateJson, data = {}, options = {}) {
  ...
  if (options.cacheKey) {
    ...
    if (cached) {
      logger.info('[Template Renderer] Using cached HTML');
      return cached;
    }
  }
  ...
  if (template.html !== undefined) {
    ...
    rendered = scrubTemplateArtifacts(rendered);
    
    // Log rendering for debugging
    logger.info('[Template Renderer] Rendered HTML length:', rendered.length, 'characters');
    if (rendered.length < 100) {
      logger.warn('[Template Renderer] WARNING: Rendered HTML is very short');
      logger.info('[Template Renderer] Rendered preview (first 500 chars):', rendered.substring(0, 500));
    }
    
    // Inline images only when explicitly requested
...
<body>`;
    ...
    template.layout.blocks.forEach(block => {
      if (block.type === 'text') {
        const content = scrubTemplateArtifacts(Mustache.render(block.content || '', data));
        ...
        const content = scrubTemplateArtifacts(Mustache.render(block.html || '', data));
