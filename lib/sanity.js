...
export function portableTextToHtml(blocks = []) {
  ...
  const toImageHtml = (block) => {
    ...
    // IMPORTANT: append ?fm=<ext> or Sanity CDN returns content-type: application/json,
    // which makes browsers block the cross-origin image via ORB (ERR_BLOCKED_BY_ORB).
    const fmt = (ref.match(/-(webp|jpg|jpeg|png|gif)$/i) || [, 'webp'])[1].toLowerCase();
    const url = `${SANITY_CDN}/${assetPath}?fm=${fmt}`;
