...
// Credit pack definitions (mapped from pricing.js)
const CREDIT_PACKS = {
  10: { pack_id: 'purchase_10', credits: 10 },
  25: { pack_id: 'purchase_25', credits: 25 },
  50: { pack_id: 'purchase_50', credits: 50 },
  100: { pack_id: 'purchase_100', credits: 100 },
};

/**
 * Purchase credits - Add ledger entry for credit purchase
 * @param {number} userId - User ID
 * @param {number} packId - Pack ID (10, 25, 50, 100)
...
export async function purchaseCredits(userId, packId, meta = {}) {
  ...
  try {
    ...
    // Idempotency check: If stripe_payment_intent_id is provided, check for existing entry
    if (meta.stripe_payment_intent_id) {
      const existingPurchase = await client.query(
        `SELECT id, delta, created_at FROM credit_ledger 
         WHERE user_id = $1 
         AND source = $2 
         AND meta->>'stripe_payment_intent_id' = $3
         LIMIT 1`,
        [userId, pack.pack_id, meta.stripe_payment_intent_id]
      );
      
      if (existingPurchase.rows.length > 0) {
        await client.query('COMMIT');
        const existingEntry = existingPurchase.rows[0];
        console.log(`[Idempotency Hit] Credit purchase already processed for payment_intent ${meta.stripe_payment_intent_id} (user: ${userId}, pack: ${packId}, ledger_id: ${existingEntry.id}) - preventing duplicate credit issuance`);
        
        return {
          success: true,
          purchase_id: existingEntry.id,
          added_credits: existingEntry.delta,
    ...
    const result = await client.query(
      ...
      [userId, pack.credits, pack.pack_id, JSON.stringify(meta)]
    );
    ...
    await client.query('ROLLBACK');
    console.error('[Credit Engine] Purchase error:', error);
...
export async function consumeCredits(userId, cost, readingId = null, meta = {}) {
  ...
  try {
    ...
    if (!snapshotResult.rows || snapshotResult.rows.length === 0) {
      await client.query('ROLLBACK');
      console.error('[Credit Engine] Snapshot not found after creation for user:', userId);
      return {
        success: false,
    ...
    console.error('[Credit Engine] Consumption error:', error);
    console.error('[Credit Engine] Error details:', {
      userId,
      cost,
      code: error.code,
...
export async function getCreditBalance(userId) {
  try {
    ...
    console.error('[Credit Engine] Balance query error:', error);
    return {
      balance: 0,
      snapshot_balance: 0,
...
export async function issueFreeDailyCredits(userId) {
  ...
  try {
    ...
    if (todayCreditsResult.rows.length > 0) {
      await client.query('ROLLBACK');
      console.log(`[Idempotency Hit] Free daily credits already issued today for user ${userId} (ledger_id: ${todayCreditsResult.rows[0].id}) - preventing duplicate credit issuance`);
    ...
    await client.query('ROLLBACK');
    console.error('[Credit Engine] Free credit issuance error:', error);
    return { success: false, error: error.message };
  } finally {
    client.release();
  }
...
export async function issueSubscriptionCredits(userId, credits, subscriptionTier, meta = {}) {
  ...
  try {
    ...
    // Idempotency check: If invoice_id is provided, check for existing entry
    if (meta.invoice_id) {
      const existingSubscription = await client.query(
        `SELECT id, delta, created_at FROM credit_ledger 
         WHERE user_id = $1 
         AND source LIKE 'subscription_%'
         AND meta->>'invoice_id' = $2
         LIMIT 1`,
        [userId, meta.invoice_id]
      );
      
      if (existingSubscription.rows.length > 0) {
        await client.query('COMMIT');
        const existingEntry = existingSubscription.rows[0];
        console.log(`[Idempotency Hit] Subscription credits already issued for invoice ${meta.invoice_id} (user: ${userId}, tier: ${subscriptionTier}, ledger_id: ${existingEntry.id}) - preventing duplicate credit issuance`);
    ...
    await client.query('ROLLBACK');
    console.error('[Credit Engine] Subscription credit issuance error:', error);
    return { success: false, error: error.message };
  } finally {
    client.release();
  }
...
export async function refundCredits(userId, amount, reason, meta = {}) {
  ...
  try {
    ...
    // Idempotency check: If stripe_charge_id is provided, check for existing refund
    if (meta.stripe_charge_id) {
      const existingRefund = await client.query(
        `SELECT id, delta, created_at FROM credit_ledger 
         WHERE user_id = $1 
         AND source = 'refund'
         AND meta->>'stripe_charge_id' = $2
         LIMIT 1`,
        [userId, meta.stripe_charge_id]
      );
      
      if (existingRefund.rows.length > 0) {
        await client.query('COMMIT');
        const existingEntry = existingRefund.rows[0];
        console.log(`[Idempotency Hit] Refund already processed for charge ${meta.stripe_charge_id} (user: ${userId}, amount: ${amount}, ledger_id: ${existingEntry.id}) - preventing duplicate refund`);
        
        return {
          success: true,
          refunded_credits: existingEntry.delta,
          ledger_id: existingEntry.id,
          idempotent: true
        };
      }
    ...
    await client.query('ROLLBACK');
    console.error('[Credit Engine] Refund error:', error);
    return { success: false, error: error.message };
  } finally {
    client.release();
  }
...
export async function addCreditsDirectly(userId, amount, source = 'admin_adjustment', meta = {}) {
  ...
  try {
    ...
    await client.query('ROLLBACK');
    console.error('[Credit Engine] Add credits error:', error);
    return { success: false, error: error.message };
  } finally {
    client.release();
  }
...
export async function cleanupExpiredCredits() {
  try {
    ...
    console.error('[Credit Engine] Cleanup error:', error);
    return { cleaned: 0 };
  }
}
