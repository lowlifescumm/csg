...
export async function refundCredits(userId, amount, reason, meta = {}) {
  ...
  try {
    ...
    return {
      success: true,
      refunded_credits: Math.abs(result.rows[0].delta),
      ledger_id: result.rows[0].id
    };
  } catch (error) {
    await client.query('ROLLBACK');
...
/**
 * Seed signup bonus credits into the ledger (the authoritative balance source).
 *
 * Signup previously only wrote to the legacy `credits` table, which the
 * balance endpoints and access-control layer never read. New users therefore
 * saw a 0 balance and could not start any credit-gated reading. This writes
 * the signup bonus directly into `credit_ledger` so the balance reflects it.
 *
 * Idempotent: a repeated call for the same user is a no-op.
 *
 * @param {number} userId - User ID
 * @returns {Promise<{success: boolean, added_credits?: number, already_seeded?: boolean, error?: string}>}
 */
export async function seedSignupCredits(userId) {
  const client = await pool.connect();

  try {
    await client.query('BEGIN');

    const credits = Number.isFinite(FREE_CREDITS.SIGNUP) ? FREE_CREDITS.SIGNUP : 0;
    if (credits <= 0) {
      await client.query('ROLLBACK');
      return { success: false, error: 'INVALID_AMOUNT' };
    }

    const source = 'free_signup';

    // Idempotency guard: never double-seed a signup bonus.
    const existing = await client.query(
      'SELECT id FROM credit_ledger WHERE user_id = $1 AND source = $2',
      [userId, source]
    );
    if (existing.rows.length > 0) {
      await client.query('ROLLBACK');
      return { success: true, already_seeded: true };
    }
    ...
    // Update (or create) the cached snapshot.
    await client.query(
      `INSERT INTO user_credit_snapshot (user_id, balance, updated_at)
       VALUES ($1,
         COALESCE((SELECT SUM(delta) FROM credit_ledger WHERE user_id = $1 AND (expires_at IS NULL OR expires_at > NOW())), 0),
    ...
    await client.query('ROLLBACK');
    logger.error('[Credit Engine] Signup seed error:', error);
    return { success: false, error: error.message };
  } finally {
    client.release();
  }
