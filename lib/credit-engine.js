...
export async function consumeCredits(userId, cost, readingId = null, meta = {}) {
  ...
  try {
    ...
    // Lock user's snapshot row for update (prevents concurrent consumption)
    ...
    const breakdownResult = await client.query(
      `SELECT 
         ...
         COALESCE(SUM(CASE WHEN source LIKE 'purchase_%' THEN delta ELSE 0 END), 0) as purchased_credits,
    ...
    const freeCredits = parseInt(breakdown?.free_credits || 0);
    const purchasedCredits = parseInt(breakdown?.purchased_credits || 0);
    const subscriptionCredits = parseInt(breakdown?.subscription_credits || 0);
    const consumedCredits = Math.abs(parseInt(breakdown?.consumed_credits || 0));
